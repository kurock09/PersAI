// ADR-074 Slice T1 — pure renderer for the per-turn "presence" developer-tail
// block.
//
// The presence template lives in `promptDocuments.presence` (see the API-side
// `CompilePromptConstructorService`). It is a raw template string with four
...
//
// All four fields are ALWAYS rendered (per ADR T1 hard constraint #4); when a
