  ...
  return {
    ...
    description: resolveToolDefinitionDescription(
      ...
      [
        ...
        "Use create_data_document (create-only) for native spreadsheets, Word documents, or data-driven PDFs built from structured/tabular data — set outputFormat to xlsx (default), docx, or pdf. The output is generated by a sandboxed program, so its size is not limited by your reply length; when the user attaches a source file (including a PDF to convert to Word/Excel), the worker mounts the raw file into the sandbox and the program reads it directly. Do not paste source content into prompt.",
    ...
    inputSchema: {
      ...
      properties: {
        descriptorMode: {
          ...
          description:
            "Document operation mode. Use create_data_document for native spreadsheets (xlsx), Word documents (docx), or data-driven PDFs built from structured/tabular data (create-only)."
        },
        prompt: {
          type: "string",
          description:
