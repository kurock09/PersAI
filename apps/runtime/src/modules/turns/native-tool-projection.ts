    ...
    inputSchema: {
      ...
      properties: {
        action: {
          ...
          description:
            'Use "describe" to load the full tool contract. Use "list_profiles" first. If a saved profile already exists for the target site, never call "login" again — use "snapshot"/"act" for normal work. Call "open_live" only when the user explicitly asks to view that saved session; a visible window does not block assistant operations and is not itself a reason to request user help. Call "login" only when "list_profiles" shows no profile for that site origin. A completed login usually lasts on that device until the site expires cookies. Runtime chooses the backend: saved-profile work uses the local browser bridge, while public no-profile snapshot/pdf/screenshot requests may use a headless backend. Do not choose or narrate the backend yourself. Profile-backed work is assistant-owned while a command runs: the page is locked for user input and shows "Assistant is working!" on hover/tap if visible. Only a structured needs_user_action result means a real CAPTCHA, verification, payment, or other user-only checkpoint was detected; the runtime opens the view and PersAI shows the user a Done banner, so stop browser calls and wait for the user\'s next turn instead of retrying or calling open_live. "snapshot" inspects a page; "act" performs bounded browser operations and returns fresh page text plus page.elements (and page.extracted when extract ops ran). Profile-backed text snapshots and acts may return page.elements with up to ' +
