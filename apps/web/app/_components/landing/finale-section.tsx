...
export async function LandingFinaleSection() {
  ...
  return (
    <LandingSection
      id="finale"
      // Mirrors the Hero's vertical structure exactly: same min-height, same
      // flex-center, same top/bottom padding rhythm. This makes the page open
      // and close on the same beat — content lands in the visual middle of
      // the viewport, not in the upper third.
    ...
    >
      <div className="mx-auto flex max-w-3xl flex-col items-center text-center">
        ...
        <p className="mt-7 text-[10px] font-semibold uppercase tracking-[0.26em] text-text-subtle">
          {t("eyebrow")}
        </p>

        <h2 className="mt-5 leading-[1.05] tracking-[-0.04em] sm:mt-6">
        ...
        <p className="mt-6 max-w-[34rem] text-sm leading-relaxed text-text-muted sm:text-base">
          {t("body")}
        </p>

        <div className="mt-9 flex w-full max-w-[18rem] flex-col items-stretch gap-2.5 sm:max-w-md sm:flex-row sm:justify-center sm:gap-3">
          <Link
        ...
        <p className="mt-7 text-[11px] leading-relaxed text-text-subtle/85">{t("note")}</p>
      </div>
    </LandingSection>
  );
}
