...
export function AssistantSettings({
  ...
  const [personalizationOpen, setPersonalizationOpen] = useState(initialSection === "memory");
  const [personalizationView, setPersonalizationView] = useState<"main" | "memory">(
    initialSection === "memory" ? "memory" : "main"
  );
  const [changeRoleOpen, setChangeRoleOpen] = useState(initialSection === "role");
    ...
    <>
      <div className="flex h-full min-h-full flex-col">
        ...
        <AssistantSettingsDialogShell
          ...
          title={personalizationView === "memory" ? t("memory") : t("personalization")}
          onClose={() => {
            if (saving) {
              return;
            }
          ...
          leading={
            personalizationView === "memory" ? (
              <button
                type="button"
                onClick={() => setPersonalizationView("main")}
                className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-text-muted transition-colors hover:bg-surface-hover hover:text-text"
                aria-label={t("personalizationBack")}
              >
            ...
            personalizationView === "main" ? (
              <div className="flex justify-end gap-2">
                ...
                <ActionButton
                  ...
                  className="h-9 min-w-[8.5rem] justify-center"
                />
              </div>
            ) : null
          }
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-border/80 bg-surface px-3.5 py-2.5 text-base text-text placeholder:text-text-subtle outline-none transition-colors focus:border-border-strong md:text-sm"
                />
              </div>
