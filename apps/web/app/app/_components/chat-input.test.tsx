...
describe("ChatInput", () => {
  ...
  it("renders a live voice button when enabled and composer is empty", () => {
    render(
      <ChatInput
        onSend={vi.fn()}
        onTranscribeVoice={vi.fn(async () => "")}
        onStop={vi.fn()}
        isStreaming={false}
        liveVoice={{ enabled: true, onStart: vi.fn() }}
      />
    );

    expect(screen.getByRole("button", { name: "liveVoice.start" })).toBeInTheDocument();
  });

  it("hides the live voice button when omitted or disabled", () => {
    ...
    expect(screen.queryByRole("button", { name: "liveVoice.start" })).toBeNull();
    ...
    rerender(
      <ChatInput
        ...
        onStop={vi.fn()}
        isStreaming={false}
        liveVoice={{ enabled: true, onStart: vi.fn(), disabled: true }}
      />
    );
  ...
  it("calls liveVoice.onStart when the live button is clicked", () => {
    const onStart = vi.fn();
    ...
    render(
      <ChatInput
        ...
        onStop={vi.fn()}
        isStreaming={false}
        liveVoice={{ enabled: true, onStart }}
      />
    );

    fireEvent.click(screen.getByRole("button", { name: "liveVoice.start" }));

    expect(onStart).toHaveBeenCalledTimes(1);
  });

  it("does not show the live voice button while streaming or while recording", async () => {
    const { rerender } = render(
      <ChatInput
        onSend={vi.fn()}
        onTranscribeVoice={vi.fn(async () => "")}
    ...
    expect(screen.getByTitle("voiceMessage")).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByTitle("voiceMessage"));
