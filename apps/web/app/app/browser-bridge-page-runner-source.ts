export const PAGE_RUNNER_SOURCE = String.raw`(input) => {
  ...
  for (const [index, operation] of (input.operations ?? []).entries()) {
    try {
      switch (operation.kind) {
        case "click": {
          getIndexedElement(operation.selector, operation.matchIndex).click();
  ...
  return {
    ...
    needsUserAction: /(captcha|recaptcha|hcaptcha|cloudflare|challenge|payment|pay now|card number|cvv|3-d secure|3ds|verification code|otp|капча|оплат|карта|смс-код)/i.test(snapshot.content)
  };
}`;
