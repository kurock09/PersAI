...
describe("WebRuntimeTurnClientService", () => {
  ...
  test("includes chatMode on web runtime sync and skill-routing-check bodies", async () => {
    ...
    globalThis.fetch = (async (_input: URL | RequestInfo, init?: RequestInit) => {
      ...
      const isSkillCheck = capturedBodies.length === 2;
      return new Response(
        JSON.stringify(
          isSkillCheck
            ? { requestId: "runtime-request-skill" }
            : {
                requestId: "runtime-request-1",
                sessionId: "runtime-session-1",
                assistantText: "native hello",
    ...
    try {
      ...
      const turnInput = {
        ...
        workspaceId: "workspace-1",
        userMessageId: "user-msg-1",
        userMessage: "analyze the project pack",
        attachments: [],
        chatMode: "project" as const,
        deepMode: true
      };

      await service.execute(turnInput);
      await service.checkSkillRouting(turnInput);

      assert.equal(capturedBodies.length, 2);
      assert.equal(capturedBodies[0]?.chatMode, "project");
      assert.equal(capturedBodies[1]?.chatMode, "project");
    } finally {
      globalThis.fetch = originalFetch;
    }
  });
