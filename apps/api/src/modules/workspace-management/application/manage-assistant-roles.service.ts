  ...
  async putCurrentRole(
    ...
    for (let attempt = 1; attempt <= MAX_ROLE_ASSIGNMENT_ATTEMPTS; attempt += 1) {
      const outcome = await this.prisma.$transaction(async (tx) => {
        ...
        const assistantRows = await tx.$queryRaw<LockedAssistantRoleRow[]>(Prisma.sql`
          SELECT
            "id",
            "user_id" AS "userId",
        ...
        if (assistant.roleId !== expectedCurrentRoleId) {
          return { kind: "retry" as const, currentRoleId: assistant.roleId };
        }
