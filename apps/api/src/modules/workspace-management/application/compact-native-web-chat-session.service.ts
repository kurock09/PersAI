...
import type { RuntimeCompactionRequest, RuntimeCompactionResult } from "@persai/runtime-contract";
...
export class CompactNativeWebChatSessionService {
  async execute(input: CompactNativeWebChatSessionInput): Promise<RuntimeCompactionResult> {
    ...
    const request: RuntimeCompactionRequest = {
      ...
      instructions: this.normalizeOptionalText(input.instructions)
    };

    const response = await this.postJson(
      new URL("/api/v1/turns/compact", baseUrl).toString(),
      request,
