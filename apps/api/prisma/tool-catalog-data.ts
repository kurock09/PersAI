...
export const TOOL_CATALOG: ToolCatalogEntry[] = [
  ...
  },
  {
    ...
    modelDescription:
      "Schedule actions for both user-visible reminders and hidden assistant follow-ups.",
    modelUsageGuidance:
      'For create, choose EXACTLY ONE explicit kind. `kind="user_reminder"` = an unconditional user-visible reminder; it requires `title`, `reminderText`, and exactly one schedule (`runAt`, `delayMs`, `everyMs`, or `cronExpr`). Use this for requests like "remind me in 10 minutes", "ping me tomorrow", or "daily check-in". `kind="assistant_check"` = a hidden background check; it requires `title`, `actionType`, a NON-EMPTY `actionPayload`, and exactly one schedule. Use this only for conditional tasks like "if FX goes above X, ping me" or "if I do not write космос in 2 minutes, ping me". When an `assistant_check` later fires, it must evaluate the payload and then, only if the condition matched, create a separate `kind="user_reminder"` with a short human-facing `reminderText`. Do not send instruction-like text such as "If in 2 minutes..." as a user reminder; reminderText must be the final message the user will actually see. For ONE intent, create EXACTLY ONE scheduled_action in this turn. Respect explicit "don\'t remind me" or paused/cancelled signals, avoid spamming, and phrase user-facing reminders as low-pressure offers. Prefer taskId from an earlier list result when pausing/resuming/cancelling; if taskId is unavailable, use titleMatch to resolve one current task by title.',
    capabilityGroup: "workspace_ops" as ToolCatalogCapabilityGroup,
    toolClass: "utility" as ToolCatalogToolClass,
    policyClass: "plan_managed"
  },
