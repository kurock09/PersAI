...
export const TOOL_CATALOG: ToolCatalogEntry[] = [
  ...
  },
  {
    ...
    modelDescription:
      "Schedule actions for both user-visible reminders and hidden assistant follow-ups.",
    modelUsageGuidance:
      'Pick audience by what is actually scheduled. UNCONDITIONAL future ping ("remind me / kick me in N minutes / schedule a daily check-in") → audience="user" with reminderText. The user WILL receive that ping. CONDITIONAL recheck ("watch FX rate, ping me if it goes above X" / "check my project progress in a day, ping only if I stalled") → audience="assistant" with actionType + a non-empty actionPayload describing the condition; that hidden turn will fire later, evaluate, and itself create an immediate audience="user" follow-up only if the condition matched. Even if the user phrased the request as "поставь себе/schedule yourself" — that is just colloquial; pick the audience by the SEMANTICS, not the wording. For ONE intent, create EXACTLY ONE scheduled_action in this turn — never both audiences for the same goal. Background assistant actions MUST NOT directly message the user; they can only schedule a downstream audience="user" action. Respect explicit "don\'t remind me" or paused/cancelled signals, avoid spamming, and phrase user-facing reminders as low-pressure offers. For create, title, audience, and exactly one schedule are required: runAt, delayMs, everyMs, or cronExpr. For audience="assistant" you MUST provide a non-empty actionPayload describing what to evaluate (otherwise the backend will coerce the row to audience="user"). Prefer taskId from an earlier list result when pausing/resuming/cancelling; if taskId is unavailable, use titleMatch to resolve one current task by title.',
    capabilityGroup: "workspace_ops" as ToolCatalogCapabilityGroup,
    toolClass: "utility" as ToolCatalogToolClass,
    policyClass: "plan_managed"
  },
