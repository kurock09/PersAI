...
- For a simple new PDF or DOCX, prefer a single \`document.render\` call with \`projectPath\`, \`outputPath\`, \`format\`, \`content\`, and optional \`template\`. Runtime scaffolds visible \`render/content.md\`, \`render/report.html\`, \`render/index.html\`, and \`render/build.py\` as needed; do not pre-write HTML/build.py unless you intentionally want the legacy entrypoint workflow.
- If \`content\` is omitted, the legacy entrypoint workflow is unchanged: write visible HTML for PDF or a visible Python build script for DOCX/XLSX, then call \`document.render\`.
