...
export const TOOL_CATALOG: ToolCatalogEntry[] = [
  ...
  },
  {
    ...
    modelDescription:
      "Schedule actions for both user-visible reminders and hidden assistant follow-ups.",
    modelUsageGuidance:
      'For create, choose EXACTLY ONE explicit kind. `kind="user_reminder"` = an unconditional user-visible reminder; it requires `title`, `reminderText`, and exactly one schedule (`runAt`, `delayMs`, `everyMs`, or `cronExpr`). Use this for requests like "remind me in 10 minutes", "ping me tomorrow", or "daily check-in". `kind="assistant_check"` = a hidden background check; it requires `title`, `actionType`, an `actionPayload` JSON object describing the condition (the runtime auto-derives a minimal payload from `title` if you omit it, so prefer to include a real one but never refuse to schedule because of it), and exactly one schedule. Use this when the user asks for a background/hidden/quiet check or phrases it conditionally, for example "поставь себе фоновую задачу", "тихо проверь", "если через 2 минуты...", or "if X happens, ping me". When an `assistant_check` later fires, it must evaluate the payload and then, only if the condition matched, create a separate `kind="user_reminder"` with a short human-facing `reminderText`; otherwise it should stay silent and MUST NOT create another `kind="assistant_check"` from inside that background run. Do not send instruction-like text such as "If in 2 minutes..." as a user reminder; reminderText must be the final message the user will actually see. For ONE intent, create EXACTLY ONE scheduled_action in this turn. CRITICAL: never tell the user "я поставил", "scheduled", "task created", or any equivalent confirmation unless you can see a tool result with `action="created"` from this same turn. If the result was `action="skipped"` (any `reason`) or you never received a successful result, say honestly that scheduling failed and what the obstacle was — do NOT pretend it worked. Do NOT first ask "хочешь чтобы я поставил?" when the user already said "поставь" / "set"; map the request straight to the right `kind` and call the tool once. Respect explicit "don\'t remind me" or paused/cancelled signals, avoid spamming, and phrase user-facing reminders as low-pressure offers. Prefer taskId from an earlier list result when pausing/resuming/cancelling; if taskId is unavailable, use titleMatch to resolve one current task by title.',
    capabilityGroup: "workspace_ops" as ToolCatalogCapabilityGroup,
    toolClass: "utility" as ToolCatalogToolClass,
    policyClass: "plan_managed"
  },
