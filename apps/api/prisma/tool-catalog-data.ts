...
export const TOOL_CATALOG: ToolCatalogEntry[] = [
  ...
  },
  {
    ...
    modelUsageGuidance:
      'Never use this tool for describing an image, OCR, solving a task from an image, or answering "what do you see". Use the available chat images: prefer the current user message attachment when present, otherwise use a recent reusable chat image already in context. With one available image, edit that image. With multiple images, set `sourceImageAlias` to the image being edited and use optional `referenceImageAlias` only as a visual guide for style, appearance, makeup, color, lighting, or background cues from another available chat image. If the user says things like "make it like the second photo", "как на втором фото", or similar, map those roles to the available image aliases and ask a clarifying question instead of guessing when the roles are still unclear. If the user asks to remove the background or return a transparent cutout/sticker/icon/logo/PNG with alpha, set background="transparent". Never claim the edit is done, ready, visible, attached, or sent unless this same turn produced a successful `image_edit` tool result or an explicit delivered artifact/result you can rely on. If you have not called `image_edit`, do not pretend the image was edited. If the tool result later says `action="deferred"`, that means the edit was accepted into the async media lane and the edited image is NOT finished in this turn yet. In that case, briefly say the edit is in progress and will arrive separately when ready; do not describe the final edited image as already created, visible, attached, or sent.',
    capabilityGroup: "knowledge" as ToolCatalogCapabilityGroup,
    toolClass: "cost_driving" as ToolCatalogToolClass,
    requiredCredentialId: "tool_image_generate",
    policyClass: "plan_managed"
  },
  {
    ...
    modelUsageGuidance:
      'Use this only when the user explicitly wants a generated video, animation, or clip. When the user directly asks you to make, animate, create, or generate a video, call this tool immediately instead of explaining the planned call. Never print `video_generate(...)`, JSON arguments, or a fenced code block as a substitute for the tool call. You may optionally guide the video with one current or recent chat image as a first-frame style or appearance reference by setting `referenceImageAlias` to one available reusable image alias. Do not use this tool for editing an existing video or for answering questions about an image. If the tool result later says `action="deferred"`, that means the request was accepted into the async media lane and the video is NOT finished in this turn yet. In that case, briefly say it is in progress and will arrive separately when ready; do not describe the final video as already created, visible, attached, or sent.',
    capabilityGroup: "knowledge" as ToolCatalogCapabilityGroup,
    toolClass: "cost_driving" as ToolCatalogToolClass,
    requiredCredentialId: "tool_image_generate",
    policyClass: "plan_managed"
