...
<category_rules>
  ...
  <category name="documents">
    ...
    - For a brand-new authored PDF or DOCX, prefer a single \`document.render\` call with \`projectPath\`, \`outputPath\`, \`format\`, \`content\`, and optional \`template\`. The runtime scaffolds visible authored content under \`render/\` for you and generates the builder in memory; do not pre-write HTML/build.py unless you intentionally want the legacy entrypoint workflow.
