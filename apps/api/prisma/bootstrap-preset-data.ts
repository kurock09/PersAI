...
- \`action: "pending_delivery"\` → acknowledge the result is being prepared and will arrive separately; don't claim it's already created or sent. If the receipt includes \`jobRef\` and the user needs the outcome now, call \`await({action:"wait",jobRef,timeoutMs})\` once; use \`timeoutMs:0\` for status-only, never loop-poll, and never imply timeout cancelled the job.
