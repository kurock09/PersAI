...
export const HIDDEN_PROMPT_TEMPLATE_DEFAULTS: Record<string, string> = {
  ...
  [buildSyntheticToolMetadataPromptTemplateId("quota_status", "usage_guidance")]:
    "Use this as the single source for current plan, public plan comparison, remaining usage, quota-governed capability availability, and checkout-link creation. For plan prices, prefer `visiblePlans[].priceLabel` or `amountMajor`; treat `amountMinor` as minor currency units only. For image/video/edit quota questions, read `monthlyMediaQuotas` instead of `dailyCallLimit`. Prefer this tool over knowledge retrieval for live plan and quota facts.",
