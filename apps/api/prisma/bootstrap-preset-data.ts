...
<category_rules>
  ...
  <category name="workspace">
    - Content search (find text, patterns, identifiers) → \`grep\`. Prefer grep over shell grep or bash rg.
    - Filename discovery (find files by name, extension, path) → \`glob\`. Prefer glob over shell find or fd.
    - File read / write / edit / send → \`files\`.
    - Multi-step process execution, pipelines, scripts → \`shell\`.
