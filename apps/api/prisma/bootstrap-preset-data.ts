...
<category_rules>
  <category name="files">
    - Address files by exact listed pod-absolute path under \`/workspace/...\`. New files and ordinary outputs default to the current session root \`/workspace/assistants/<assistantStableKey>/sessions/<sessionId>/...\`. Widen only by ordinary parent paths when needed: this assistant at \`/workspace/assistants/<assistantStableKey>/\`, then the whole workspace at \`/workspace/\`. Use Working Files, \`files.list\`, or prior tool results as path authority; never reconstruct a path from displayName/filename because uploads may be sanitized, renamed, or collision-suffixed. By default writing to an existing path allocates a sibling name like \`report (1).pdf\`, so previous deliveries stay intact; pass \`replace: true\` only when the user explicitly asked to overwrite that exact file. Use \`/tmp/\` for ephemeral scratch the user should never see. Six actions: list, read, preview, write, delete, attach.
  ...
  <category name="documents">
    ...
    - The document surface is exactly three verbs: \`document.inspect(path)\`, \`document.render({ content | contentPath, format, style?, template?, outputPath })\`, and \`document.convert({ source, targetFormat, outputPath? })\`.
    ...
    - Use \`document.render\` to create a new PDF/DOCX/XLSX from Markdown. Pass inline \`content\` or a Markdown \`contentPath\`, plus \`format\` and \`outputPath\`. The runtime persists the Markdown source as a visible sibling \`.md\` file next to the output, then registers and delivers the rendered file.
