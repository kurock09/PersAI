...
- **D13 addendum — found and fixed the actual root cause: plain Browserless `429 Too many requests` queue pressure, not session death/crash/fingerprint:** immediately after D12 deployed, the parent agent drove the same live PersAI chat session directly (not the founder) to get first-hand evidence with working logs. `snapshot`/`click` worked cleanly (60 elements returned, confirming D11 live). `type` failed — the model's own real-time diagnosis ("Browserless backend crashes on `page.type()`") was wrong; `kubectl logs` on the D12-instrumented pod showed the real cause instantly: `[persistent-bql] transport failure status=429: Too many requests`. Per Browserless's own docs, `429` means the plan's concurrency queue is full — plausibly the true explanation behind the entire "gateway 502"/"type crashes" arc reported across this whole session, not any of the session/proxy/fingerprint hypotheses explored earlier. Fixed with `fetchJsonWithRateLimitRetry` (2 retries, 600ms/1500ms backoff) wired into both the persistent BQL and ephemeral `/function` paths — absorbs transient queue pressure per Browserless's own guidance instead of surfacing every `429` as a hard failure; exhausting the retry budget still raises a normal gateway error. New unit test covers both paths. If `429`s persist after this ships, the account's Browserless plan concurrency limit itself needs raising (not evaluated here).
- **D12 addendum — closed the real observability gap: browser tool failures were completely silent in both runtime and provider-gateway logs:** a live borscht-shopping test on the deployed D11 image reported 3 straight `act` failures (catalog "+" button `add-spin-button` never found after scroll) plus a "gateway 502" symptom, but `kubectl logs` across both `runtime` and `provider-gateway` pods showed nothing for the whole window — no request trace, no error. Root cause, three parts: **(1)** `RuntimeBrowserToolService.executeToolCall`'s top-level catch (and `executeListProfiles`'s) swallowed every failure into a structured `browser_failed` result with zero logging; fixed with `logger.warn(...)` (action/url/profile/real error message) plus a `logger.log(...)` request-trace line before the outbound call. **(2)** D10's logging used `debug`, filtered out under this cluster's real `LOG_LEVEL=info`; fixed by switching to `logger.log(...)`. **(3)** D10 only logged BQL `errors[]` on a 200 response — the `!response.ok` transport branch (both persistent BQL and ephemeral `/function` paths) and the ephemeral script-level fatal-error branch threw with zero logging, very plausibly the actual "gateway 502"; fixed with `warn` logs before both throws. On the "+" button: D11's cap-60/visibility fix was already live for this test and the button still never appeared after three scroll attempts — this points away from "cap still too low" toward a possible hover-only-render UI pattern (not proven; none of our operation kinds dispatch a hover event). Pragmatic fallback stays the D10-proven path: click into the product card, use the large add-to-cart control.
- **D11 addendum — element-extraction visibility ranking + act chaining guidance, from model-observed friction on the same live Lavka session (verified against code, not taken at face value):** two follow-ups after the D10 add-to-cart success. **Real defect:** the 25-element cap in both extraction scripts ran plain document-order `querySelectorAll` with no visibility filtering, so header/nav/cart-icon chrome filled the cap ahead of catalog/product content — live-confirmed on Lavka's search results. Fixed by filtering to currently-visible elements (`getClientRects().length > 0` + non-hidden computed style) before the cap, raising `MAX_RUNTIME_BROWSER_INTERACTIVE_ELEMENTS` from 25 to 60, and fixing a parity bug where the ephemeral `/function` path hardcoded `25` instead of reading the shared constant. **Not a code defect:** the model asked for a way to chain `[click, type, press, click]` in one `act`, believing one-op-per-call was required — the `operations` array (up to 6 steps, including `wait_for_selector`) already runs every step in a single call on both execution paths; the gap was the schema description never taught this. Fixed by rewriting the `operations` description to explicitly teach chaining with `wait_for_selector` as the bridge after any step that opens new content.
...
- D11's visibility filter is a heuristic (visible/non-hidden), not true viewport/relevance ranking; a page with more than 60 visible interactive elements ahead of the useful ones can still miss them — if that matters live, the next step is viewport-distance ranking, not raising the cap again.
...
- D12's logging fix has not yet been live-verified against a fresh failure (no redeploy/retest happened before this addendum was written) — the very next live test is what proves it actually works.
...
- D13 is implemented and unit-tested locally but not yet re-verified live end-to-end — the very next live test (the same borscht/multi-ingredient scenario) is what proves it actually resolves the recurring failures.
- The catalog "+" button hover-only-render question (D12) is still open — D13 fixed a separate root cause (429 queue pressure), not the "+"-button-not-in-DOM issue.
...
**Next step:** after commit/push and next deploy, redo the same Lavka/borscht scenario and confirm the D13 429-retry fix actually eliminates the recurring "gateway 502"/"type crashes" failures. If `429`s still show up in `kubectl logs` even after retry, check the Browserless dashboard's plan/concurrency settings — the account may genuinely need a higher concurrency tier. Separately, the catalog "+" button question (D12) is still unresolved: once 429s are out of the way, retry the scroll+click sequence and see if the button appears — if it still never does, decide whether to add a hover-simulating operation kind or keep the proven product-page fallback as the standard path. Also confirm the model actually chains multi-step `act` calls (D11) instead of one-op-per-call. Residential proxy remains an open account/plan question (see D10) — check the Browserless dashboard plan/add-ons page before spending more engineering time on it. Then continue the rest of live Browserless acceptance: cold-session re-auth reopening, modal-first web re-auth, and Telegram web-login fallback copy.
