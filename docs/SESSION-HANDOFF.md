...
**Next step:** after commit/push and next deploy, directly re-verify (browserleaks or equivalent) that a persistent profile call now returns a non-datacenter IP and a non-headless UA, and that a guessed-selector `act` call returns a warning instead of a 502 — then continue the rest of live Browserless acceptance: persistent `page.elements`, cold-session re-auth reopening, modal-first web re-auth, and Telegram web-login fallback copy.
