...
- **ADR-072 T15-7 closeout + Step 16 boundary cleanup:** repo-truth docs now close `T15-7` and `Step 15` honestly after the already-landed quota/prompt/admin follow-through. The stray `Sandbox tool matrix` row was moved out of the Step 15 tool-slice ledger into the `Step 16` section, and `persai_workspace_attach` is now explicitly treated as migration-only truth whose future replacement is post-Step-16 attach-by-ref follow-through on top of canonical `fileRef` authority rather than part of the Step 16 sandbox matrix itself.
