...
- **Early turn router rollout + docs sync:** the active path now uses a configurable early `turn_routing` layer with deterministic precheck plus an optional cheap classifier, driven by admin-managed `routingFastModelKey`, `routerPolicy`, and `router_classifier` prompt state. Explicit smart/deep-mode turns still clamp to `premium_reply` or `reasoning`, but active repo-truth docs no longer describe hidden `route_control` as the current routing core.
