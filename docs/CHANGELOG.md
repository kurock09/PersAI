...
- **ADR-072 Step 14 Telegram native text/group cutover:** `HandleInternalTelegramTurnService` now sends Telegram direct/group turns straight to native `apps/runtime` execution with proper `RuntimeConversationAddress` identity and shared canonical-history hydration. The remaining Telegram-only `/compact` and compaction-hint seam was removed instead of being carried into the target architecture; shared compaction/tooling is explicitly deferred to Step 15.
