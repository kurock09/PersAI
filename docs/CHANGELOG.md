...
- **ADR-072 T15-3a shared runtime hardening:** `apps/runtime` now keeps ordinary model-visible Step 15 tool exposure honest: sync turns project only the real machine-readable compaction tools, stream turns disable model-visible tool exposure instead of running a hidden buffered tool loop, and shared compaction can reuse the accepted-turn lease through `RuntimeCompactionRequest.heldLease`. `knowledge_search`, `knowledge_fetch`, `persai_workspace_attach`, and `persai_tool_quota_status` now remain gated off the ordinary model-visible path until real PersAI-native executors exist. This honestly closes `T15-3a`: helper tools stay dark instead of blocking the shared-runtime hardening milestone, and any later helper exposure/admin follow-through moves to `T15-7`. Focused runtime tests plus runtime/runtime-contract typechecks cover the new projection, stream, and lease-safety behavior.
