...
- **`route_control` model-only hardening + docs sync:** hidden `route_control` no longer carries tool guidance or availability semantics; it is now documented and enforced as reply-model selection only. The same follow-through also clamps explicit smart/deep-mode turns so a user-selected premium path can escalate to `reasoning` but cannot silently fall back to `normal_reply`, and active repo-truth docs now describe that contract consistently across ADR/program handoff surfaces.
...
- **ADR-073 Economics Slice B cache-routing baseline:** the active OpenAI text path now carries provider-side prompt-cache hints instead of waiting for a full prompt-assembly rewrite before any live economics gain is possible. `packages/runtime-contract` now supports text-request `promptCache` metadata, `apps/runtime` now derives stable OpenAI `prompt_cache_key` plus explicit in-memory retention hints for ordinary turns, hidden `route_control`, and shared compaction, and `apps/provider-gateway` now forwards those hints to the OpenAI Responses API while keeping other providers untouched. A live `persai-dev` probe also confirmed that the current active path can already realize large cached-input reuse on repeated long prefixes (`0 -> 6912 cachedInputTokens` on back-to-back `gpt-5.4-mini` requests), while ADR-073 remains honest that stable prompt-block materialization, broader helper-family minimization, and first-class observability are still open follow-through inside Slice B.
- **ADR-073 Economics Slice A ready + live hardening follow-through:** repo-truth docs now mark the first economics slice as landed on the active path: plan-scoped `normal` / `premium` / `reasoning` / hidden `system-tool` / optional `retrieval` slots, hidden `route_control`, explicit deeper-thinking mode, and honest per-call `input` / `cached input` / `output` accounting are now treated as complete, with `Slice B` prompt-cache-first context architecture as the next active economics step. The same follow-through pack also hardens live behavior by normalizing Unicode-hyphen model ids across control-plane/runtime/provider-gateway seams, teaching the browser to ignore `NEXT_PUBLIC_API_BASE_URL=localhost` in live and fall back to same-origin `/api/v1`, fixing the `route_control` structured-output schema (`reason` is now required) while hiding raw planner-failure text from the user-visible model path, accepting `admin.plan.delete` in the dangerous-action step-up challenge parser, polishing the chat header toggle, chat badge wording, and Clerk avatar fallback behavior, and clarifying in ADR-073 that knowledge-search helper work should use the plan-owned `retrieval` slot with the smallest viable task-specific prompt instead of paying for the full conversational system shell.
