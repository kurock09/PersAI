...
- **ADR-072 T15-6 native `image_edit` source/reference refinement:** `image_edit` now stays honest on live user flows: `apps/runtime` projects it only for explicit edit intent, current-turn image hydration now exposes numbered `image #1`, `image #2`, and so on for model clarity, the native request contract now carries `sourceImageIndex` plus optional `referenceImageIndex`, and runtime/provider-gateway/OpenAI execution now support both one-image edits and bounded two-image `source + reference` edits while skipping ambiguous multi-image calls instead of guessing. Focused provider/runtime tests and repo-truth docs were refreshed, while `video_generate` remains the next `T15-6` follow-through.
