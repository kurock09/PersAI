...
- **ADR-074 Slice V1 — Voice DNA scaffold (code-landed locally, awaiting live-dev smoke):** the assistant's "voice" is no longer a free-form trait list interpolated into an opaque `soul` template. There is now an editable Voice DNA layer composed of (a) bilingual archetypes stored in the new `persona_archetypes` Postgres table, (b) a pure modulator that nudges the archetype with the user's slider traits per turn, and (c) a richer `soul` template that consumes structured Voice DNA placeholders. Four founder-co-authored archetypes (`warm-quiet`, `playful-sharp`, `calm-deep`, `dry-witty`) ship as compiled defaults in `apps/api/prisma/persona-archetype-data.ts` with both `ru` and `en` copy for label, description, openings (allowed and forbidden), per-emotion behaviors, silence rule, and 2 micro-examples each. The defaults are seeded via insert-only upsert (`ManagePersonaArchetypesService.ensureDefaults`), so admin edits are preserved across deploys; a per-archetype "Reset to default" admin action restores the compiled baseline on demand. The runtime resolves Voice DNA per turn through `modulateVoiceDna({ archetype, traits, locale })` in `apps/api/src/modules/workspace-management/application/voice-dna-modulator.ts` — sliders act as conservative nudges around the archetype baseline (verbosity → sentence length, initiative → pace, playfulness/warmth → irony floor and cap at 90), and a universal forbidden-openings list is dedup-merged with archetype-specific bans before injection. Snapshotting at publish time captures both `snapshotArchetypeKey` and a raw locale-agnostic `snapshotVoiceDna` JSON on `assistant_published_versions`; at materialization, the live archetype is preferred (so admin edits propagate to existing assistants on the next config bump) and the snapshot is the deletion-fallback. The setup wizard and assistant settings now persist `archetypeKey` on the draft; the 9 existing front-end personality presets are mapped to one of the 4 backend archetypes via `PRESET_KEY_TO_ARCHETYPE` in `apps/web/app/app/_components/assistant-persona.ts` so the user-visible UI does not change in V1 — a dedicated 4-archetype picker is intentionally a follow-up V1.x slice. Admin UI: `/admin/presets` gains a new "Voice DNA Archetypes" section that lists every archetype, lets the operator edit the full bilingual JSON in place, and includes per-archetype "Save" and "Reset to default" controls (`PATCH /api/v1/admin/persona-archetypes/:key` and `POST /api/v1/admin/persona-archetypes/:key/reset`). Both admin actions bump config generation so the next ordinary turn picks up the new Voice DNA on materialize. New unit tests in `apps/api/test/voice-dna-modulator.test.ts` cover locale resolution, neutral 50-trait baseline preservation across all 4 archetypes × 2 locales, archetype `defaultTraits` flow-through, slider nudge directions and clamps, forbidden-openings invariants per archetype × locale (universal + archetype-specific present, no duplicates), and en-fallback when ru localization is missing on a partial archetype. New Prisma migration `20260421120000_adr074_v1_persona_archetypes_foundation` creates `persona_archetypes`, adds `assistants.draft_archetype_key`, and adds `assistant_published_versions.snapshot_archetype_key` + `snapshot_voice_dna`; the migration is applied automatically by the `api-migrate` PreSync hook on the next Argo CD sync. Live acceptance against `persai-dev` (`pnpm smoke:run --scenario emotional-long --scenario chitchat-short`) is the remaining gate before V1 is marked closed.
