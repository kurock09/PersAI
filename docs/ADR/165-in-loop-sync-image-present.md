...
5. ADR-162 ConversationalPublish / catch-up remains the chat-present path for
   ordinary deferred jobs that finalize outside the open USER_TURN bubble, but
   a live catch-up bubble suppresses its classic attachment strip until the
   continuation itself becomes terminal.
...
| D6.1 | **2026-07-26 repair.** Deferred enqueue stores `sourceToolCallId` (chat tool call id). Open-turn present resolves `afterToolCallId` from artifact `producingToolCallId` **or** that enqueue field so italic receipts bind. Live UI also shows orphan attachments as receipts when placement is missing. Deferred media tool-end does **not** clear the “Generating image” chip (await / next tool / empty open-jobs snapshot does). Stick-to-bottom must not re-fire on attachment-only message patches.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| D6.2 | **2026-07-27 repair, amended by ADR-166 follow-up.** Nest must `@Inject(WebChatLiveTurnPresentService)` into media/document completion delivery (no `@Optional() … \| null = null` — `Pick\|null` metadata resolved to null and always took settle-without-chat). Open-turn `ensureCompletionMessage` pins `openTurnAttempt.assistantMessageId`, never the ack row. Web: italic receipts only when `liveInlineMediaReceipts` on ordinary USER_TURN live bubbles (including cold status reattach); catch-up / async-cont never set that flag, and while they are still streaming or reconciling they also suppress the classic attachment strip so no committed-looking image renders below the live cursor. One classic bottom strip appears only after terminal commit. Clearing deferred-media / await live chips sets `streamingTextActive: false` so status falls back to thinking («Думаю»), not a blank pre-answer slot. |
...
Catch-up bubbles do not gain ordinary live inline receipts; while live they
suppress their attachment strip until terminal commit. It does not reintroduce
a sync-image tool exception.
