...
Slices 1-4 are sequential within memory. Slice 10 is a memory follow-up after Slices 1-4 and should complete before broad tool-surface cleanup that relies on memory-search guidance. Slice 6 depends on Slice 5. Slice 9 depends on Slice 5 (alias examples) and should align its memory-search instructions with Slice 10. Slices 7 and 8 are independent.
