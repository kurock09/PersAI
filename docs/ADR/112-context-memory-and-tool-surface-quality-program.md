...
- Pre-cleanup, `previous attachment #N` meant different things in Working Files (all attachment kinds) vs the old image-tool recent attachment resolver (images only). The active runtime path now uses Working Files sticky aliases as the single tool/worker attachment namespace.
...
- **Slice 4 - Safe memory backfill (A backfill).** DONE (2026-06-07) — API-only two-phase admin backfill landed for one assistant at a time. `ManageAdminMemoryBackfillService` computes a bounded preview (`MAX_BACKFILL_SCAN = 1000`, `SAMPLE_LIMIT = 20`) over active rows only (`forgottenAt = null`, `supersededAt = null`) and exposes two candidates: (1) legacy episodic `core` rows where `memoryClass = core` but NOT (`durability = identity` AND `stability = stable`) -> previewed for reclassification to `contextual`; (2) `web_chat` rows whose stored summary is obviously non-durable via `isObviouslyNonDurableMemorySummary(summary)` -> previewed for soft prune. Apply re-loads fresh state, requires step-up action `admin.memory_backfill.apply`, prunes trivial `web_chat` rows first via `markForgottenById`, then reclassifies the remaining legacy `core` rows via `reclassifyMemoryClassById(..., "contextual")`, with prune winning if a row matches both targets. One admin audit event is appended per apply. New admin routes: `POST /api/v1/admin/memory-backfill/preview` and `POST /api/v1/admin/memory-backfill/apply`.
  - Acceptance: dry-run output reviewed before any mutation; live problem assistant verified lighter after backfill. MET at code/test level for the dry-run + step-up protected apply surface.
- **Slice 5 - Unified file references + sticky aliases (C + D).** DONE (2026-06-08) — Working Files now uses one model-facing sticky namespace: `file #N` for all reusable files and `image #N` for image-capable entries. Labels are assigned from first appearance and preserved across hydration/tool-loop merges, so adding a newer file no longer renumbers older files. Recency/role is expressed separately through markers such as `current source` / `last delivered result`, while `fileRef` remains the hidden execution identity. The developer section is now titled `## Working Files`, renders `createdAt | author | sticky label | filename | markers | microdescription`, keeps same-name files disambiguated by short hash, and preserves PDF-specific document anchors (`DOC_CURRENT_SOURCE` / `DOC_LAST_DELIVERED_PDF`) when older files exceed the cap. Runtime files/document/image tool guidance and native projection examples now teach sticky aliases instead of legacy current/previous/found/read/listed aliases. Focused tests cover stable labels, hydration, tool-loop discovery merging, files/document resolution, image tool consumers, and native projection.
  - Acceptance: adding a file does not renumber existing files; `image_edit`/`video_generate`/`files`/`document` resolve sticky labels; no parallel alias scheme left behind. MET.
- **Slice 6 - Developer-block clarity (C).** DONE (2026-06-08) — collapsed into the Slice 5 follow-through plus a residual cleanup audit. The model-facing section title/tool vocabulary/example alignment landed with Slice 5; the follow-up removed the remaining unused `showCurrentTurnImageOrdinals` / direct-attachment counter plumbing from runtime hydration. Source audit confirms no `## File history` or dead current-turn image ordinal path remains in active runtime turn assembly.
  - Prompt-context follow-up (2026-06-09): Telegram developer context is now one centralized `## Channel Context` section for both private and group Telegram turns. Group-only privacy cautions stay group-only, and Telegram audio/voice-like inbound attachments add a concise TTS voice-reply preference hint when available.
  - Prompt-context follow-up (2026-06-09): `Working Files` no longer prints the column-format legend, carries shorter file-tool guidance, and suppresses historical audio/voice attachments from the default prompt surface. Current-turn audio and discoverable reusable files remain reachable through the existing tool path.
  - Acceptance: the Working Files block remains provider-native and cache-safe, and no dead current-turn image ordinal path remains. MET.
- **Slice 7 - Background jobs visibility (E).** DONE (2026-06-09) — runtime-facing job visibility now separates true in-progress work from post-completion delivery state. API still preserves web continuity truth (`activeMediaJobs` / document chips keep `completion_pending` and `ready_for_delivery`), but runtime `openMediaJobs` / `openDocumentJobs` now exclude those statuses. A new internal `RuntimeTurnRequest.jobDeliveryUpdates[]` seam carries both `finalizing_delivery` and `delivered_recently` items into one compact runtime-owned `## Job Delivery Updates` section, which explicitly says generation/rendering is already finished, delivery may still be catching up, and recently delivered items may already be visible in chat history. This closes the turn-start race where `completion_pending` was previously shown as "open" and prevents stale "still generating" answers after worker completion. The model-facing section also makes the audio-lane decision explicit: async audio generation is not an active lane; current voice replies use synchronous `tts`.
  - Acceptance: model no longer says "still generating" after worker completion; completion is visible promptly. MET.
- **Slice 8 - Background model-call prompt hygiene (F).** DONE (2026-06-09) — background model-call surfaces are now cleaner and more observable without changing the user-visible chat path. Runtime bundles now expose the background-task evaluator prompt as `backgroundTaskEvaluation`, while `heartbeat` remains a compatibility alias for existing prompt-template storage/materialized bundles. The skill-state classifier prompt now includes the caller's `checkReason` (`background_bootstrap`, `background_cadence`, or `foreground_activation`) and carries a dedicated `skill_state_classifier` provider classification. Runtime background-task evaluation keeps the real background-task path on `background_task_evaluation`, while quota-advisory follow-ups now use `quota_advisory_evaluation` so provider logs/ledger sources are distinguishable. Compaction exhausted follow-ups no longer call the heavy two-phase background-task evaluator at all; after the existing compaction eligibility/suppression checks, API creates a static grounded notification intent from durable compaction facts.
  - Acceptance: background prompts are consistent and observable; advisory no longer runs a 2-phase pipeline. MET.
- **Slice 9 - Tool descriptor + instruction hygiene (G).** DONE (2026-06-08) — runtime model-facing tool projection was cleaned without changing execution architecture. Document guidance now consistently uses `docId` (camelCase) and documents it as UUID-only, not a Working Files alias. `memory_write` descriptor text no longer exposes ADR-id implementation cruft. `scheduled_action` projection is sanitized to user-visible reminders only and no longer surfaces stale hidden-follow-up / `assistant_check` policy copy. `background_task` projection no longer advertises a ghost `update` action when stale policy text mentions it; the schema remains `create | list | pause | resume | cancel`. `image_generate`, `image_edit`, `video_generate`, and `document` share one concise pending-delivery honesty helper. Video/persona guidance consistently says `Settings -> Characters`. `tts` now has one short rule: if the user asks for a spoken reply or voice note, call `tts`, and do not claim audio exists unless the same turn returns `action='generated'`. Focused projection tests now reject stale worker vocabulary, legacy alias teaching, `doc_id`, ghost `update`, and inconsistent Characters wording. Internal compatibility names and historical test fixtures that are not model-facing were left alone by design.
  - Acceptance: the cited model-facing descriptor defects are resolved or explicitly deferred as internal/non-model-facing compatibility; clean tools stay clean; focused projection/scheduled-action/TTS tests updated to live truth. MET.
  - Live video-tool follow-up (2026-06-09): `video_generate` now says `talking_avatar` is only for explicit spoken-avatar narration with non-empty `speechText`; ordinary video, image animation, silent/no-speech gestures, smiles, winks, air kisses, and cinematic/product/fashion clips stay on `cinematic`. Empty `speechText` validation points the model to cinematic or to ask for/provide a real script. Async media-job execution treats model-correctable video request errors (`invalid_arguments`, bad reference/portrait aliases, missing voice/persona/config) as non-retryable. Media/document worker payloads now use Working Files sticky aliases as the single source of truth; the old separate recent-image/recent-document-source resolvers were removed instead of retained as hidden fallback namespaces.
- **Slice 10 - Model-judged two-tier memory + clean contextual window (A/B follow-up).** DONE (2026-06-08). Replace raw `web_chat` turn dumps with a single model-judged memory entry path: the model chooses `long` (stable durable facts/decisions) or `short` (recent episodic working context) or writes nothing. `long` maps to the always-on core layer; `short` maps to contextual memory with existing decay/lifecycle. The dynamic contextual block becomes a bounded recency window over active short memory, rendered consistently every turn when non-empty, without lexical pre-turn search. Older short memory remains available through explicit memory search/tool use when the model needs it. Add an idle-session safety pass for short sessions that did not auto-compact: after 20 minutes idle, run one compact-model memory-extraction pass over only the unprocessed message delta, advance a per-session memory-extraction watermark, and only run again after enough new messages accumulate and another idle window elapses. Auto-compaction shares that same watermark after its own memory extraction, but compaction remains primarily about cheaper continuation without losing chat context. Remove the raw `web_chat` auto-write path and heuristic trivial-turn gate; do not keep a legacy dual path or shadow mode in production. Align the active-router memory path so the same item cannot be injected both as contextual memory and retrieved knowledge.
  - Landed: `memory_write` now exposes `long` / `short` and maps them deterministically to core/contextual; raw `web_chat` post-turn memory writing is removed from the active path; the contextual block is now a bounded recency window over active short-memory rows instead of lexical search; active-router retrieval locally de-dupes memory items already rendered in that short-memory block; focused tests cover long/short routing, recency ordering/budget, and removal of the raw web-chat path.
  - Completed follow-up: `runtime_sessions.memoryExtractionWatermark` is the durable per-session extraction seam; auto-compaction extraction now evaluates only the compacted prefix after that watermark and advances the shared watermark on successful/no-item completion; a new API idle-session enqueue scheduler reuses the existing background-compaction queue to run `idle_extract` only after 20 idle minutes plus `>=10` new hydratable messages after the watermark; provider/incomplete failures leave the watermark unchanged and are retry-bounded by the existing job scheduler; focused runtime/API tests cover first-pass advance, no-op repeat, under-threshold skip, delta-only idle extraction, compaction/watermark sharing, and bounded provider-failure behavior.
...
- **Alias migration (Slice 5):** RESOLVED at code level — Working Files, `files`, `document`, `image_edit`, `video_generate`, and native projection now use the sticky `file #N` / `image #N` scheme. Residual risk is test/doc drift, mitigated by the independent audit follow-up that removed the second media alias path and corrected stale contract/docs truth.
