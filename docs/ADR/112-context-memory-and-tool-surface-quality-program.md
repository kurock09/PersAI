...
- **Slice 1 - Memory capture schema + classification + web-chat gate (A Layer 1).** Add the model-judged durability/stability signal to the write path; route core/contextual/skip on it; gate web-chat writes; tighten `memory_write` guidance. No heuristic deciding layer.
...
- **Slice 2 - Memory retrieval/render quality + single runtime-owned framing (A retrieval/render + B).** Relevance threshold + sort, semantic dedup vs core, grouping by kind, single header/framing owned by runtime; keep `cacheRole` volatile placement.
  - Acceptance: contextual block is one grouped block; low-relevance noise (e.g. greeting matches) dropped; provider cache stability unchanged (ADR-110 tests green).
- **Slice 3 - Memory normalization + consolidation + lifecycle (A Layers 2-4).** Embedding-based dedup/merge; periodic background reflection extending auto-extract; supersession and decay.
  - Acceptance: near-duplicate memories merge; a consolidation pass demotes/prunes; superseded facts stop resurfacing.
...
- **Consolidation cost (Slice 3):** the reflection pass is an LLM call; it must stay off-band (background) and rate-limited like compaction, not on the user turn.
