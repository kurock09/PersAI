...
1. **Hallucinated identity persisted as a fact.** On `long-session-200` turn 22 ("как меня зовут?") the assistant answered "Тебя зовут **General**" instead of the planted "Алекс" from turn 1, and `AutoExtractToMemoryService` then persisted that as a `fact / core` durable-memory row ("Пользователь называется General"). Hypothesis: the assistant's own display name on the test assistant is "General"; the rolling synopsis lost the user's name early, and the model substituted the assistant identity. The auto-extract LLM took the conversation at face value. The fix is in M2.1: tighten the auto-extract system prompt to forbid extracting identity facts unless the user asserted them in their own message in the compacted slice, not in the assistant's reply.
