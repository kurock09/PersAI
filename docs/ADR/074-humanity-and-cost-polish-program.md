...
- **Status:** Queued (not deferred to a future ADR — owned by ADR-074, sequenced after M3 lands and after at least one week of live evidence on how often the model uses M3's `closeOpenLoop: true` flag and how often the implicit dedup-overwrite path resolves loops cleanly).
...
- **Status:** Queued (founder picked option **C-trimmed** with cooldown **D2 = 12h** on 2026-04-22, immediately after M3 `persai-dev` smoke acceptance landed; sequenced before T1 because Telegram lacks a "new chat" affordance and M3 alone never re-fires there).
