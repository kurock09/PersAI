...
**One quality polish kept on the M2.1 todo list (NOT a blocker for closing M2 — the slice acceptance is clean on the renamed profile):** even though the "General" symptom went away with the `userName` rename, the underlying class of issue is still real: `AutoExtractToMemoryService` will happily extract a fact that already lives in the stable user-profile prefix (name, language, role, preferences) for any user whose profile field happens to surface in conversation. The fix in M2.1 is to pass a compact rendering of the relevant user-profile fields into the auto-extract LLM prompt as a "known about user — do NOT re-extract these" guard, and to treat any extracted candidate that matches a profile-derived fact as a `dedupSkipped` outcome (same counter the existing M1 dedup uses). This is not urgent — a duplicate row in durable memory is bounded by the same hard cap and dedup pipeline as any other row — but it keeps M2's "human friend voice" promise honest: a friend would not write down what they already know about you.

**One smoke-harness improvement kept on the M2.1 todo list (NOT an M2 production bug — this is a test infrastructure artefact):** when the three scenarios are run sequentially on a single shared `assistantId`, the `multi-session-continuity` "Маша" persona's facts (Atlas / Helio / retention) bleed into the `long-session-200` run because durable memory is keyed by `(assistantId, userId)` and is by design cross-thread within an assistant. This is the correct production behavior (one assistant per user, memory carries across that user's threads), but it pollutes acceptance for smoke runs. Fix surfaces in either (a) one assistant per smoke scenario, or (b) a `forget all` warmup step in the harness; both options stay deferred as a smoke-harness improvement.

The slice is therefore considered closed: cutover, replace-not-concatenate synopsis, off-band scheduler, auto-extract through M1 dedup, prompt-cache rename, and the UI pressure-banner suppression are all live and measurably better; the two M2.1 polish items above are tracked and will be addressed without re-opening M2.
