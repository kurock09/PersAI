...
Live smoke acceptance was then run against the founder's `persai-dev` `Custom` plan (`compactionTriggerThreshold = 8000`, `targetContextBudget = 70000`, `keepRecentMinimum = 4`, `autoCompactionWeb = true`) on assistant `b635d40d-ced6-428d-a68b-7395463b2db9` for the three M2-relevant scenarios. All deltas vs. the pre-M1 baselines under `scripts/smoke/baselines/`:

- `chitchat-short` — 8/8 OK; total tokens **−22.96 %** (43 776 vs 56 821); p95 latency `−1041ms`; no auto-compaction expected (well below the 8 k threshold).
- `multi-session-continuity` — 12/12 OK across both sessions; total tokens **−30.63 %** (113 218 vs 163 201); p95 latency `−1776ms`; session-2 used `knowledge_search ×6` → relevance-retrieval path is wired.
- `long-session-200` — 29/29 OK; total tokens **−41.29 %** (246 011 vs 419 021); p95 latency `−5933ms` (no regression despite compaction now firing — the headline M2 acceptance signal); per-turn input stayed bounded at `~5–8 k` for ordinary turns with retrieval-driven spikes only on the explicit recall turns.

Off-band auto-extract is observed live: a `GET /api/v1/assistant/memory/items` against the test assistant after the smoke runs returns rows with `sourceType: "memory_write"` + `kind: fact | preference | open_loop` + `memoryClass: core | contextual`, written by `AutoExtractToMemoryService` through the M1 internal `writeMemory` path (`assistant.memory_write_duplicate` audit fires for dedup hits). `Auto-compaction triggers: 0` in every smoke `summary.json` is **expected** because the harness reads `RuntimeTurnReceipt.autoCompaction`, which is the in-band path the cutover deliberately removed; the durable-memory listing is the off-band proof.
...
**Two acceptance regressions discovered during smoke and explicitly deferred to a follow-up slice (NOT blockers for closing M2 itself, because the cutover mechanics are correct and measurably wins; these are content-quality issues to be solved in M2.1 or folded into M3):**

1. **Auto-extract duplicated a user-profile fact into durable memory.** On `long-session-200` turn 22 ("как меня зовут?") the assistant answered "Тебя зовут **General**" instead of the planted "Алекс" from turn 1, and `AutoExtractToMemoryService` then persisted that as a `fact / core` durable-memory row ("Пользователь называется General"). **Diagnosis (founder-confirmed 2026-04-22 evening):** "General" is the founder's actual registered user-profile name on `persai-dev`, which already sits in the stable system-prompt prefix; "Алекс" was a smoke-fixture identity planted in-conversation by turn 1 of the scenario. The model resolved the conflict in favor of the system-prompt profile (architecturally correct — the user profile is a more stable signal than a single in-conversation turn), so the *answer* is fine. The real M2 bug is narrower: auto-extract wrote into durable memory a fact that is already permanently present in the stable user-profile prefix, which is by definition redundant. The fix in M2.1 is therefore **not** "tighten identity extraction" but **"do not re-extract facts already known from the user profile"**: pass a compact rendering of the relevant user-profile fields (name, language, top-level role/preferences) into the auto-extract LLM prompt as a "known about user — do NOT re-extract these" guard, and treat any extracted candidate that matches a profile-derived fact as a `dedupSkipped` outcome. Renaming the founder's profile is explicitly **NOT** the right fix — it would risk live-assistant reset, would not solve the class of issue (any profile field could be duplicated this way), and would only paper over the smoke confound below.
2. **Cross-scenario bleed-through within the same `assistantId`.** On `long-session-200` turn 24 ("над какой продуктовой задачей я работаю?") the assistant answered with "Atlas / Helio / Retention" — those facts are from the `multi-session-continuity` scenario's user persona ("Маша"), which ran 5 minutes earlier on the same assistant. This is technically expected M1+M3 behavior on a single shared `assistantId` and is fine for real users (one assistant per user), but it pollutes acceptance for smoke runs that share an assistant id across scenarios. Fix surfaces in either (a) one assistant per smoke scenario, or (b) a `forget all` warmup step in the harness; both options are deferred to the next session as a smoke-harness improvement, not an M2 production bug.

The slice is therefore considered closed: cutover, replace-not-concatenate synopsis, off-band scheduler, auto-extract through M1 dedup, prompt-cache rename, and the UI pressure-banner suppression are all live and measurably better; the two open quality items are tracked above and will be addressed without re-opening M2.
