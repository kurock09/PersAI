...
- **Slice 3 LANDED** — 2026-06-20 (per-session pod reuse, idle-TTL reaper, GCS-keyed workspace snapshot). **Warm pool re-opened + landed 2026-06-21** — see the "Cold-start reliability + warm-pool re-opened" addendum below; the earlier DEFER note is superseded.
...
- **Slice 5 LANDED** — 2026-06-21 (commit `6613091b`) — PDF cutover: WeasyPrint render via `render_html_to_pdf` toolCode; PDFMonkey fully removed; pdfmonkey→sandbox provider rename + Prisma migration; chunked pipeline removed; enhanced print CSS always on; truncation detector improved
...
- **Delivery MIME allowlist opened to allow-all.** `DEFAULT_RUNTIME_SANDBOX_POLICY.artifactMimeAllowlist` is now `["*/*"]` and a data migration (`20260621120000_adr123_open_delivery_mime_allowlist`) rewrites every plan's stored `sandboxPolicy.artifactMimeAllowlist` to `["*/*"]` so operators never hit a per-plan delivery-MIME wall. This does **not** widen the safety boundary: the true ceiling remains the persist-time media validation (`media-security-policy.ts` — `ALLOWED_MEDIA_MIMES` + `DANGEROUS_FILE_EXTENSIONS`), which runs on every produced artifact regardless of the delivery allowlist. `RuntimeFilesToolService.assertMimeAllowed` treats `*/*` (and `*`) as the allow-all sentinel; any other non-empty list stays an exact-match allowlist. **D3 unchanged.**
- **Egress allowlist broadened (still deny-all + allowlist — the Claude model, not open internet).** Added the GitHub ecosystem (`.github.com`, `.githubusercontent.com`) to `egressProxy.allowedDomains` alongside PyPI + npm for git clone / VCS pip installs / release+raw fetches. The deny-all default and the secret-exfiltration protection of D3 are preserved; open/unfiltered egress was explicitly rejected.

**Cold-start reliability + warm-pool re-opened — 2026-06-21 (founder directive; supersedes the Slice 3 "warm pool DEFERRED" note):**
The Slice 3 deferral assumed cold-start latency was "~2–4s gVisor". Live evidence invalidated that: the first exec command after idle took **~104s** because the `sandbox-pool` autoscaled a fresh node and then pulled the multi-GB exec image (Python + doc/data stack + Chromium). Worse, **four budgets were all sized for the warm path and conflated pod startup with command runtime**, so a cold start _failed_ rather than waiting: (1) `waitForPodRunning` used `policy.maxProcessRuntimeMs` (15s default) as the pod-ready deadline; (2) the control-plane running-job watchdog (`maxProcessRuntimeMs + grace`, ~30s) force-failed the job as `sandbox_execution_timeout`; (3) the runtime client `resolveCompletionTimeoutMs` (~40s) abandoned the job; (4) the queued-stale threshold (45s). Two-track fix:

- **Reliability (correctness).** Introduced a dedicated **pod-provisioning budget** — `SANDBOX_EXEC_POD_PROVISION_BUDGET_MS` (control plane) and `RUNTIME_SANDBOX_POD_PROVISION_BUDGET_MS` (runtime), default **240s** — threaded through `waitForPodRunning`, the running-job watchdog, and the runtime completion timeout. `maxProcessRuntimeMs` is now used **only** as the per-command runtime cap, never as a pod-startup deadline. A cold start now _succeeds_ (waits once) instead of failing.
- **Latency (warm pool, re-opened).** A `sandbox-exec-prepull` **DaemonSet** keeps the exec image pulled+cached on every sandbox node (declarative, Helm), paired with a **≥1 minimum node** on `sandbox-pool` (operational `gcloud node-pools update`, documented in the RUNBOOK). Together the first command after idle drops from ~100s to seconds. The prepull pod only sleeps, holds no secrets, and never runs model code. This is the lightweight realization of the D4 warm pool; the earlier deferral is closed.

**Document-job reliability + retired-pricing cleanup — 2026-06-21 (founder directive; documents pre-deploy hardening):**
A failed live test document exposed two issues:

- **Transient sandbox infra failures were mis-classified as non-retryable.** A render's sandbox job hit a workspace-push flake (`process_spawn_failed` — "Workspace push closed without success"). `exec-pod-bridge.createBridgeError` defaulted `blocked=true`, so the sandbox recorded the job as `blocked` (a policy block) rather than `failed`. The document worker's `renderHtmlToPdf` maps `retryable = (status === "failed")`, so a `blocked` status made it non-retryable and the job died after 1/5 attempts, surfacing the misleading `document_artifacts_missing`. **Root fix:** bridge errors are **operational/retryable by default** (`blocked=false`); only genuine non-retryable blocks pass `blocked=true` explicitly (stdout/stderr resource-limit; the command-runtime-budget `process_timeout` where retrying the same command would time out again). Spawn / pod-provisioning / push failures now surface as `failed` → the async document render job retries (and, with the cold-start fix above, lands on a warm pod). This aligns `process_spawn_failed` and the provisioning `process_timeout` with the already-correct `sandbox_failed` (exec/pull) treatment.
- **Retired `document_render:pdfmonkey` pricing row removed.** Slice 5 removed PDFMonkey from code (`document_render` provider defaults are gamma-only), but the tool-path pricing **catalog merge preserves stored rows**, so the stale `document_render:pdfmonkey` row (price 8000) kept rendering in Admin > Tools. Idempotent data migration (`20260621130000_adr123_drop_retired_pdfmonkey_render_price`) strips it from the stored `tool_path_pricing_catalog`; the prod row was also cleaned live.

**Workspace-push success-detection regression fixed — 2026-06-21 (founder directive; THE "sandbox shell doesn't work" root cause):**
The 2026-06-20 "reliable workspace push" rework detected success via a sentinel (`__PERSAI_PUSH_OK__`) printed by the remote `tar` on the **same exec WebSocket that streams the tar archive on stdin**. Live ground truth (`sandbox_jobs` on `persai-dev`) showed every recent `shell`/`exec`/`render_html_to_pdf` job failing `process_spawn_failed` / "Workspace push closed without success (stderr: none)" with `exec_pod_name: null`, including one that waited 80s on an already-Running pod — i.e. not a cold-start timeout. Reproduced with the real `@kubernetes/client-node` from inside the control-plane pod against the live session pod: a 1 KB stdin payload returns the sentinel, but 200 KB / 2 MB payloads return `status=Success` (the remote command exits 0) while the exec **stdout channel is silently dropped** and the status frame races the socket close. So success could never be read off the push socket for a real (non-trivial) workspace. **Fix:** decouple transfer from success detection. The push exec streams the tar and writes a marker file only when `tar` exits 0 (`rm -f marker; tar … && printf ok > marker`), and its own stdout/status are ignored (resolve on clean close; reject only on a genuine connection failure). A **separate stdin-less verify exec** then asserts the marker — with no stdin multiplexed, its return channels are reliable (verified 4/4 with 2 MB payloads in-cluster). A missing marker is a retryable `process_spawn_failed`; the command and pull never run on a failed push. This supersedes the sentinel-on-push-stdout scheme. **Warm-pool/$ note:** the `sandbox-exec-prepull` DaemonSet provides ~no benefit at `sandbox-pool` min-nodes=0 (it co-schedules on the freshly-autoscaled node) and only helps with min-nodes≥1 (standing node cost) — left as an explicit founder cost decision, resolved in the next addendum.

**Exec pod re-keyed per assistant+workspace + warm-node trial enabled — 2026-06-21 (founder directive):**
Two coupled changes that supersede the Slice 3 "per chat-session pod" + "warm pool deferred/min-nodes=0" positions:
(1) **Reusable exec pod is now keyed by `(assistantId, workspaceId)` instead of `runtimeSessionId`.** Every chat of one assistant workspace now shares a single warm exec pod, so the second/third chat skips pod creation entirely (the previous per-`runtimeSessionId` scheme spun up a fresh pod per chat). This is safe because the pod's `/workspace` is re-pushed from the control plane on every job (pod identity is purely a warmth optimization, never a source of file truth) and the existing `assistantId+workspaceId` Postgres workspace lease already serializes all jobs for that workspace — so there is never concurrent use of the shared pod, even across chats. Pod name = `ses-<sha256(assistantId:workspaceId)[0..31]>`. The pod now carries `persai.io/assistant-id` + `persai.io/workspace-id` annotations (replacing `persai.io/session-id`); the idle-TTL reaper derives last-activity from `sandboxJob.findFirst({ where: { assistantId, workspaceId }, orderBy createdAt desc })`. Old per-session pods + the `persai.io/session-id` annotation drain naturally: without the new annotations the reaper falls back to pod-creation age and evicts them after the idle TTL. The control-plane GCS workspace snapshot (keyed by `assistantId+runtimeSessionId`) is unchanged and orthogonal — it persists ephemeral files on the control plane, not in the exec pod. Sessionless (`runtimeSessionId === null`) jobs remain ephemeral (Slice 1 behavior preserved). `runInPod`/`runInSessionPod`/`ensureSessionPodRunning`/`buildSessionPodName` now thread `assistantId`+`workspaceId` (from `RuntimeSandboxJobRequest`); unit tests updated for the new keying + annotations.
(2) **`sandbox-pool` autoscaling min-nodes 0 → 1 applied live (1-month trial)** so one sandbox node + the pre-pulled exec image stay warm and the first command after idle starts in ~2–5s instead of ~100s. The `sandbox-exec-prepull` DaemonSet stays enabled (`infra/helm/values-dev.yaml` `sandboxExec.prepull.enabled: true`) and now actually pays off (it co-schedules on the standing node). The dev cluster is **zonal** (`europe-west1-b`) — RUNBOOK §8 corrected from `--region europe-west1` to `--zone europe-west1-b`, max-nodes left at the cluster's actual 2. Founder will review the standing-node cost after ~1 month and decide whether to keep min-nodes=1.
...
- **Slice 6 — Documents mode B. [LANDED — 2026-06-21]** D7: model-writes-code Excel/DOCX/data-PDF executed in sandbox; mode A/B routing in the document worker keyed on `descriptorMode === "create_data_document"` (→ `documentType: data_document`, `provider: sandbox`). **Create-only** this slice — no revise/export wiring for data documents (a "PDF→Word" transform is simply a `create_data_document` whose source is the mounted file). Contract: `AssistantDocumentType += data_document`, `AssistantDocumentDescriptorMode += create_data_document`, `AssistantDocumentOutputFormat += xlsx, docx` (additive Prisma migration `20260621000001_adr123_slice6_data_document`); `DEFAULT_RUNTIME_SANDBOX_POLICY.artifactMimeAllowlist += ` the Office xlsx/docx MIME types. Worker `runCodeDocumentPath`: authors a single self-contained Python 3 program (no inlined source text), runs it in the sandbox `execute_document_code` toolCode, one bounded self-repair retry on non-zero exit (stderr fed back; terminal `document_code_failed` after the second failure — no loop), validates the artifact (xlsx/docx = `PK\x03\x04` ZIP magic; pdf = `%PDF-` reused), persists as `runtime_output`, soft-deletes the transient `sandbox_output`. **Two-tier source ingestion** (founder-approved supersession of inline-text): TIER 1 mounts raw source files via `RuntimeSandboxJobRequest.mountedFileRefs` into `/workspace/sources/<name>` and the prompt instructs the model to read them natively (pdfplumber/python-docx/openpyxl/pandas); TIER 2 runs a runtime-layer `pdf-parse` text-layer probe and, for scanned PDFs / images, runs the existing `document-extraction.service` OCR and mounts an `*.ocr.txt` sidecar alongside the original (the tier decision lives in the runtime worker, never a sandbox round-trip). Verify a large data document is produced without hitting the output-token ceiling.
- **Slice 7 — Tools. [LANDED — 2026-06-21]** D8: added `grep` + `glob` inline tools (control-plane `rg`/`fd`, argv-array + path containment, output/match caps, never an exec pod); rewrote `shell` `modelUsageGuidance` (proactive autonomous multi-step; search routed to grep/glob; discouragement removed); added the additive `<category name="workspace">` to `<tool_usage_policy>` (ADR-119 structure preserved). `rg`/`fd` added to the control-plane Dockerfile. Plan-managed activation/loop budget unchanged; grep/glob default `active:true` like `files`. New contract result types `RuntimeGrepToolResult`/`RuntimeGlobToolResult`; new runtime `runtime-grep-glob-tool.service.ts`; grep/glob wired into `/admin/presets`. **Deploy residual:** rebuild the control-plane `sandbox` image so `rg`/`fd` are on PATH. Verify the model autonomously uses shell/grep on a multi-step file task once deployed.
