...
#### D2a — DeepSeek durable append-only chat trace

The generic sealed compact spine and rotating full overlays remain the OpenAI
and Anthropic in-turn protocol. They are not a provider-valid DeepSeek cache
boundary: DeepSeek has no explicit marker, and its disk cache can only reuse
its own persisted, exact wire-prefix units.

For ordinary/deep DeepSeek chat, PersAI therefore owns one server-only,
durable, chat-scoped append trace. It is not public API state and is not the
canonical chat authority. It contains only exact, already model-sanitized
provider-visible events in one ordered epoch:

1. an initial stable/developer snapshot;
2. canonical user/assistant conversation entries needed by that epoch;
3. complete DeepSeek assistant tool-call + matching tool-result pairs,
   including required `reasoning_content`;
4. complete catalog `describe` result contracts; and
5. immutable context revisions for model-visible Skill, Scenario, plan,
   knowledge, files, job, delivery, preview, and forced-final facts.

Every later DeepSeek request must replay the epoch verbatim and append only
new events before its active user input or follow-up generation. A revision
uses a stable state key and explicit supersession identity; old event bytes
are never mutated. The latest revision is the only current state for that
key. A revision must never be inserted between a DeepSeek assistant
`tool_calls` message and its matching `tool` result.

Runtime owns one DeepSeek trace coordinator immediately before every provider
dispatch, including streaming, synchronous, post-final, and recovery
dispatches. It receives the already assembled current model-visible context.
Rather than requiring every Skill, Scenario, file, knowledge, job, preview,
or delivery producer to own a trace hook, the coordinator emits one
`runtime_context` revision for its complete exact mutable suffix only when its
content hash differs from the last revision. That event has one state key and
explicitly supersedes the prior revision. This is the sole append boundary for
dynamic state; no producer may independently rebuild a prior trace event.

The trace never stores raw subsystem/provider payloads, artifact identifiers,
binary content, or unbounded file bodies. It uses the same first-exposure
model sanitization already applied to tool results. It does not use the
generic compact spine or newest-three overlays. This retains a described
...
- ordinary durable chat compaction completes;
- the selected provider, DeepSeek model, published bundle/system prompt, or
  tool projection family changes;
- the user explicitly resets/starts a new chat; or
- the chat is hard-deleted (cascade deletion).

Archive/restore alone does not reset an epoch. OpenAI and Anthropic never
read, write, or depend on this trace. Their existing protocol remains
unchanged.

Before each DeepSeek dispatch, Runtime must account for the complete trace and
the selected model's output reserve. It must not silently compact, mask,
delete, or re-order trace events inside an epoch. When another provider call
cannot safely fit, the DeepSeek path must use one explicit, tested
no-more-tools/finalization behavior rather than the generic in-loop overflow
recovery that rebuilds request context.
