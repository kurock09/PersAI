...
- D9's proxy/UA-mask fixes are not yet live-verified: they are locally implemented, focused-tested, and reasoned from Browserless's own documented mutation examples, but no post-deploy browserleaks-equivalent check has confirmed a non-datacenter IP / non-headless UA on a real session yet. Do not repeat D7's mistake of treating a successful page load or a saved profile address as proof of proxy/stealth effectiveness — only a direct IP/fingerprint check counts.
