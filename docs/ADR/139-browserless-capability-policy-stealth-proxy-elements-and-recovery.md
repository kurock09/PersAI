...
### D12 — Closed the real observability gap: browser tool failures were completely silent in both runtime and provider-gateway logs (2026-07-07)

A live borscht-shopping test on the already-deployed D11 image (`provider-gateway:2c69b546...`, confirmed via `kubectl describe pod`) reported 3 straight `act` failures trying to find a catalog "+" button (`data-testid="add-spin-button"`) after scrolling, plus a "gateway 502" symptom. `kubectl logs` across both `runtime` and `provider-gateway` pods for the entire test window showed **nothing** — no request trace, no error, not even a basic access log for `/api/v1/providers/browser-action` — for either the successful or the failed calls. Root-caused by reading the actual call chain instead of guessing further:

- **Real defect 1 — `RuntimeBrowserToolService.executeToolCall`'s top-level catch swallowed every browser failure with zero logging.** Any thrown error from `executeBrowserPageAction` (which includes every `BadRequestException`/`BadGatewayException` the provider-gateway HTTP client raises) was caught, turned into a structured `browser_failed` tool result for the model, and never logged — the real reason a call failed lived only in the model-facing `warning` string, invisible server-side. `executeListProfiles`'s catch had the same gap. **Fix:** both catches now call `this.logger.warn(...)` with the action/url/profile-presence and the real error message before returning the structured result. A `this.logger.log(...)` entry line was also added right before the outbound `providerGatewayClientService.browserAction(...)` call (mirroring the existing `streamText` request-trace pattern), so every browser tool call — success or failure — now has a start marker in `kubectl logs runtime-*`.
- **Real defect 2 — D10's own logging fix used `debug`, which is filtered out under this cluster's actual `LOG_LEVEL=info` (`infra/helm/values-dev.yaml`).** The D10 per-call summary line never once appeared in any live test because of this — confirmed by grepping full pod log dumps for the entire test window and finding only NestJS route-registration output. **Fix:** changed to `this.logger.log(...)` (info) in `ProviderBrowserService`.
- **Real defect 3 — D10 only logged BQL-level `errors[]` on a 200 response; the `!response.ok` HTTP-transport branch (an actual non-2xx from Browserless, e.g. a raw 502) threw immediately with no logging at all in either the persistent BQL path or the ephemeral `/function` path.** This is very plausibly the actual "gateway 502" the model reported — with no log evidence either way. **Fix:** both `!response.ok` branches, plus the ephemeral path's separate script-level fatal-error branch (`data.error.message`), now log a `warn` line with the status and extracted message before throwing.

**On the catalog "+" button itself:** D11's visibility filter + 60-element cap was already live for this exact test (confirmed by the pod image tag) and the button still never appeared after three separate `act`+`scroll` attempts. This shifts the leading hypothesis away from "still not enough elements" — raising the cap to 100 is unlikely to help on its own. The model's own hypothesis (Lavka only mounts/reveals this per-card stepper control on a real pointer-hover event, not via scroll or programmatic DOM presence) is plausible and consistent with a common quantity-stepper UI pattern, but is **not yet proven** — none of our operation kinds (`click`/`type`/`scroll`/`wait_for_selector`/`wait_for_timeout`) dispatch a hover/`mouseover`/`pointerover` event, so this cannot be distinguished from "element genuinely absent from DOM until hover" versus any other cause without a direct live recheck now that logging actually works. Until proven, the pragmatic path stays what already works end-to-end (D10): click into the product card, use the large add-to-cart control there — one extra round trip per ingredient, but reliable.

### D13 — Live recheck with D12's logging found the actual root cause: plain Browserless `429 Too many requests` queue pressure, not a session death, code crash, or fingerprint issue (2026-07-07)

Immediately after D12 deployed, the parent agent drove the same live PersAI chat session directly (not the founder) to get first-hand evidence with working logs. Results, in order:

1. A fresh `snapshot` succeeded and returned 60 elements (visible confirmation that D11's cap-60 + visibility filter is live and working) — the earlier "gateway completely died, three 502s in a row" report from before D12 did not reproduce; the session was not actually dead.
2. `click` worked cleanly against the live profile.
3. `type` failed. The model's own real-time diagnosis ("Browserless backend crashes on `page.type()`, not profile/fingerprint") was **wrong** — `kubectl logs` on the D12-instrumented `provider-gateway` pod showed the real cause immediately: `[persistent-bql] transport failure status=429: Too many requests (requestId: ...)`. Confirmed via Browserless's own enterprise docs: `HTTP 429` on Browserless specifically means "the concurrency queue is full" (plan-level concurrency cap exceeded, requests queued and then rejected once the queue itself fills) — it is not a crash, not fingerprinting, and not session death. This is very plausibly the true explanation for the "gateway 502" and "type crashes" symptoms reported across this entire live-test arc, not any of the previously-explored infra/proxy/session hypotheses.

**Fix:** added `fetchJsonWithRateLimitRetry` in `ProviderBrowserService` — a bounded retry (2 retries, 600ms then 1500ms backoff) specifically for `429` responses, wired into both the persistent BQL path and the ephemeral `/function` path. This absorbs transient queue pressure (Browserless's own documented recommendation) instead of surfacing every `429` straight to the model as a hard tool failure. Exhausting the retry budget still surfaces as a normal `BadGatewayException` — this is a backoff, not an infinite retry loop, and does not paper over a sustained/systemic concurrency shortfall.

**Residual:** if `429`s persist even with retry, the account's Browserless plan concurrency limit itself needs raising (see Browserless dashboard → plan/concurrency settings) — this is an account-tier ceiling, not something client-side retry alone can fully absorb under sustained load. Not evaluated in this addendum; flagged as the next thing to check if `429`s keep recurring after this fix ships.
...
- D12 fixed the logging gap but has not yet been live-verified against a fresh failure (no redeploy/retest happened before this addendum was written). The next live test must confirm actual log lines appear in `kubectl logs` for both success and failure before this is considered closed.
- D12 did not resolve the catalog "+" button question — it only made the next live attempt diagnosable. If the hover-only-render hypothesis is confirmed, the follow-up would be a genuine new operation kind (dispatch `mouseover`/`pointerover` before checking for hover-revealed controls), which is out of scope for this addendum and would need its own decision before implementation.
- D13's retry is fixed at 2 attempts / 600ms+1500ms and does not read a `Retry-After` header (Browserless's JSON error body did not expose one in the observed live failures; `fetchJson`'s return type does not currently carry response headers). If `429`s persist after this fix ships, the next step is checking actual concurrency usage against the account's Browserless plan limit, not tuning the retry schedule further.
- D13 was implemented and unit-tested locally immediately after the live 429 discovery, but not yet re-verified live end-to-end — the next live test is what proves the retry actually resolves the borscht/multi-ingredient scenario.
- The catalog "+" button hover-only-render question (D12) remains unresolved — D13 fixed a separate root cause (429 queue pressure), not the "+"-button-not-in-DOM issue.
