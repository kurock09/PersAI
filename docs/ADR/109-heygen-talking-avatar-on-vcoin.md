...
Completed (2026-06-07). The operator verified the ADR-109 talking-avatar foundation and declared the program complete. All further HeyGen voice-cloning, Characters UI v2, and video-player polish work moves to ADR-111 (`docs/ADR/111-heygen-voice-cloning-and-characters-ux-v2.md`) and must not reopen this ADR.

Scope completed in this ADR: HeyGen credential and catalog truth, talking-avatar mode contract, HeyGen voice preset cache, workspace video personas, eager HeyGen avatar creation at persona POST, provider-gateway HeyGen v3 client, runtime `mode="talking_avatar"` execution, plan gating, Assistant Settings Characters management, model-facing persona/tool-description guidance, async talking-avatar delivery UX, audit-pass reconciliation, and live-fix follow-through. Historical slice details and errata remain below as the implementation ledger.

Closure note: ADR-109 intentionally keeps voice cloning out of its completed scope. ADR-111 supersedes only that deferred non-goal; it does not alter the ADR-109 historical record.
...
**Status (2026-06-07): Completed by operator verification.** ADR-109 is closed as the HeyGen talking-avatar foundation. The operator verified the landed path and accepted the remaining product polish as follow-up scope for ADR-111 rather than additional ADR-109 slices. Completion includes the credential/catalog/runtime/provider-gateway/persona/settings/tool-description/async-delivery path plus the post-slice live fixes recorded in the changelog and handoff. Further work on voice cloning, Characters UI v2, and generic video-player polish is explicitly owned by ADR-111 and must not reopen this ADR.
...
6. Voice cloning is out of scope for ADR-109; ADR-111 supersedes this deferred item as a new program.
...
- **Russian voice preset quality may not match user expectations.** Mitigation: voice cloning is now planned in ADR-111; assistant should set expectations honestly until that program lands.
...
- **Voice cloning deferral.** Users will ask. Mitigation: ADR-111 opens the voice-cloning follow-up; ADR-109 remains preset-only.
...
- [x] HeyGen credential stored under `tool_video_generate_heygen` and visible in Admin Tools.
- [x] HeyGen catalog rows configurable in Admin Runtime as video-only and talking-avatar-only.
- [x] Runtime contract carries `mode = "cinematic" | "talking_avatar"` plus speech / persona / portrait / voice fields.
- [x] Tool projection advertises the new fields only when plan toggle is on.
- [x] HeyGen voice presets cached and attached on materialization.
- [x] `workspace_video_personas` table exists with limit enforcement and VC debit on creation.
- [x] Provider-gateway HeyGen client submits / polls / downloads.
- [x] Provider-gateway creates HeyGen avatar id at persona POST; lazy create remains defensive fallback only per E12.
- [x] Runtime executes `talking_avatar` end-to-end through HeyGen and debits VC on success.
- [x] `talking_avatar` does not fallback to Runway / Kling / OpenAI.
- [x] Cinematic mode behavior is unchanged (regression test).
- [x] Plan toggle `talkingVideoEnabled` exists, gates execution, and exposes locked-with-upsell UI when off.
- [x] Assistant Settings has a workspace-scoped Characters section.
- [x] Multi-character requests are handled by model/tool-description guidance with split suggestion.
- [x] Persona archive/delete behavior is accepted as soft-delete; HeyGen-side cleanup remains an acknowledged limitation outside ADR-109.
- [x] Operator verification accepted ad-hoc/persona talking-avatar flow, plan toggle/error behavior, and insufficient-VC handling as complete for ADR-109.
- [x] Docs and changelog/handoff record the ADR-109 surface and follow-up ownership.
- [x] ADR-107 line 39-40 exception for HeyGen recorded in this ADR.
- [x] Full verification gates passed across the landed slices and follow-up fixpacks.
- [x] Live/operator verification accepted ADR-109 as complete; further polish moves to ADR-111.
