...
- Add `thinkingBudgetByLevel: { light, medium, heavy, deep } | null` end-to-end by mirroring the existing per-plan `toolBudgets` mechanism (NOT the flat model-key path): OpenAPI `AdminPlan*` schemas + regenerated `@persai/contracts`, a `thinking-budgets-policy.ts` module (defaults / parse / persist as a versioned JSON sub-document inside `billingProviderHints` / read-back), `manage-admin-plans.service.ts` wiring, a `RuntimeThinkingBudgetByLevelConfig` on the runtime contract + `AssistantRuntimeBundleRuntimeConfig`, `materialize-assistant-published-version.service.ts` resolving + conditionally spreading it onto `bundle.runtime`, and `runtime-bundle-registry.service.ts` warm-load validation. The runtime then reads `bundle.runtime.thinkingBudgetByLevel` in `turn-routing.service.ts` and passes the non-null leaves as the resolver's override map at every `resolveExecutionProfile` call site. Per-leaf validation is `null` or a non-negative integer (`0` = off), matching the resolver.
