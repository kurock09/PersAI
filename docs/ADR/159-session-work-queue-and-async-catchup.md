...
`terminated`. Provider-gateway mapped that to unknown
`provider_request_failed`, so runtime's existing retry/fallback gate rejected
it. The bounded repair classifies known transport disconnect signatures as
