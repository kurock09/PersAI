...
- **Media share path (native share sheet for authenticated chat media).** The image lightbox share button in `apps/web` now prefers `window.PersaiNative.shareMedia(payloadJson)` when the shell exposes it. The payload carries the authenticated media URL, filename/title, and current WebView user-agent. Android handles that call inside the existing `PersaiNative` JavaScript interface: it downloads the file with the WebView cookie into app cache and opens the system `ACTION_SEND` chooser via the existing `FileProvider`. iOS injects the same `shareMedia(...)` bridge from `AppDelegate.swift`, reuses the WKWebView cookie store for the authenticated download, persists the file into temporary cache, and presents `UIActivityViewController`. Browser web keeps the existing `navigator.share` / download fallback, so the product still has one web UI while the shell owns only the native file-share execution seam.
...
- **WebView is not a native UI.** Animation feel, scroll inertia, keyboard handling, and accessibility primitives are the platform WebView's, not native. For PersAI's chat-centric product this is acceptable. Where a native seam materially improves correctness without splitting the UI tree (for example authenticated media sharing), the shell may expose a small bridge and keep the decision logic in `apps/web`.
...
- retries by explicitly replacing the location with `https://persai.dev`. This re-enters the normal Capacitor `server.url` load path even on WebView versions that would otherwise reload the local error page itself; if connectivity is still down, Capacitor falls back to `offline.html` again.
...
**Mid-session (web-side).** Once the WebView has loaded `apps/web` successfully and the user starts interacting with it, network drops are detected inside React. `apps/web/app/app/_components/use-network-online.ts` is a small hook that combines `navigator.onLine` with `online`/`offline` events, plus a `recheck()` action that performs a no-cache `fetch("/api/health")` so the user can force a re-evaluation without waiting for the OS to fire `online`. The hook broadcasts recheck results across mounted instances, so a failed pre-navigation check in the sidebar also updates the root `OfflineGate`. `apps/web/app/app/_components/offline-gate.tsx` consumes that hook and renders a fullscreen overlay with the same copy/affordances as the cold-start page; the rest of the app keeps mounted state (chat scroll, draft text, recordings) under the overlay so that recovery is non-destructive — closing the overlay just resumes whatever the user was doing. `OfflineGate` is mounted once near the root in `apps/web/app/app/_components/app-shell.tsx`. Sidebar-owned app navigations (`new chat`, chat switch, admin/profile, locale reload) run the same bounded health check first; if the network is actually down, the navigation is blocked and the current screen remains mounted under the overlay. Browser Back and overlay-close behavior are intentionally not blocked. The overlay deliberately does not freeze any in-flight network call: that is `useChat`'s job (see "Single-slot pending send" below).
...
A WebView session on a flaky mobile signal has a third failure mode that neither the cold-start fallback nor the mid-session overlay covers cleanly: the request may have been sent, but the client does not yet know whether the server accepted it. For that the design layers a small state machine onto every outgoing user message. The constraint is deliberately strict: at most one message can be unresolved at a time. A second send is blocked until the first one is durably reconciled or explicitly abandoned from a confirmed-never-sent local failure state.

`ChatMessageStatus` (in `apps/web/app/app/_components/use-chat.ts`) keeps `sending` (optimistic, request in-flight) and `send_failed` (recoverable pending-send presentation), but those UI labels are not themselves the server authority. The authoritative reconciliation surface is the exact `clientTurnId` status route: `unknown`, `accepted`, `running`, `completed`, `failed`, or `interrupted`. Retry/Cancel actions branch on that durable status, not on local timing heuristics alone.

**Pre-flight gates.**

- If `navigator.onLine === false` at submit time, the user bubble enters a confirmed-never-sent local failure state immediately. No fetch is attempted. The single slot fills, the composer locks until Retry/Cancel.
- If a previous message is unresolved, `useChat.send()` is a no-op. `chat-input.tsx` reflects this with a disabled Send button, a small destructive-toned helper line, and identical Enter-to-send gating.

**In-flight watchdogs.**

- **Attachment uploads** (`stageWebChatAttachment`, `transcribeVoice`) go through `apps/web/app/app/upload-with-progress.ts`, an XHR helper that emits `progress` events and exposes two timers: a 15s **stall watchdog** (no progress event for 15s => abort with `XhrStallError`) and a 5min **hard upper bound** (`XhrTimeoutError`). The progress-based stall gate is the smart one: large genuine uploads on weak signal stay healthy as long as bytes keep moving, but a frozen connection trips the watchdog quickly. These failures enter pending-send reconciliation; they do not by themselves prove whether the logical turn was accepted upstream.
- **Stream turn** (`streamAssistantWebChatTurn`) gets a 10s **pre-headers timeout** in `useChat`. The success signal is the `onHeadersOk` callback, which fires on `response.ok` (the server has accepted the request and is now writing SSE). After headers, tool turns may legitimately stay silent for tens of seconds (image generation can run 30–60s); we do not impose a wall-clock cap on the post-headers stream. If the 10s elapses without `onHeadersOk`, the controller is aborted and the user bubble enters pending-send reconciliation rather than assuming the request never arrived.
...
- The same bubble in `send_failed` shows a small destructive `AlertCircle` plus "Not delivered", with two compact text-buttons underneath: **Retry** and **Cancel**. Those labels now mean:
  - `unknown`: never redispatch automatically; reconcile exact `clientTurnId` first.
  - `accepted` or `running`: reattach the exact turn id and keep the composer locked.
  - `completed`: hydrate the committed history and clear the pending state.
  - `failed` or `interrupted`: that exact `clientTurnId` is terminal and immutable. If the failed pending slot is text-only, explicit Retry creates one fresh logical turn while preserving the old failed canonical row. If the restored canonical row had attachments, browser `File` objects are unavailable after F5, so Retry must not silently send text-only; it preserves the failed row, restores text into the composer, unlocks pending, and requires the user to reattach files before a new send.
- **Cancel** is safe only for a confirmed-never-sent local failure. In every unconfirmed or restored-canonical case it reconciles before unlocking, and it must not clear `unknown`, `accepted`, or `running`. Retry/Cancel actions bind only to the exact current pending user message id, never to historical failed rows elsewhere in the transcript. For plain text, a confirmed-never-sent cancel restores the draft into the composer via `ChatInputHandle.setDraft` so the user does not lose what they typed. Voice/file blobs are restorable only while they still exist purely in local pending state; once server acceptance is ambiguous or durable, the user must reconcile or start a fresh retry path.

**Persistence.** Local pending-send presentation is in-memory and per-thread, but the durable logical-turn attempt is server-owned by `clientTurnId` and a restored pending slot may already be backed by a canonical server user row. On reload/F5/reattach, the client must reconcile with server truth before admitting a new send. If a restored pending send resolves to terminal `native_runtime_conflict`, it is shown as a recoverable failed pending presentation rather than as a committed assistant orphan, and Retry/Cancel never delete that canonical failed row locally.
