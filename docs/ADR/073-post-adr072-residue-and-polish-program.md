...
- **FIX 1, Slice 1.3 — browser resume reconciliation.** After live mobile/tab testing, the remaining user-visible gap was not server completion but stale client state: a phone lock or tab background could tear down the local stream, show an error after unlock, while the server had already committed the message and attachments. `useChat` now refreshes the latest active-thread history on `focus`, `visibilitychange` back to visible, and `pageshow`, replacing optimistic `local-*` bubbles with committed server messages and clearing the stale issue when history reconciliation succeeds.
...
- provider-gateway text-generation warmup now honors admin-managed OpenAI/Anthropic keys saved in PersAI's encrypted runtime-provider secret store: static `persai-runtime-secrets` env vars still win when present, but missing env vars no longer make an admin-selected provider permanently `unconfigured` if the key was saved through the admin surface
...
As of 2026-04-25, the active admin-managed catalog is capability-aware per provider: `chat`, `image`, and `video`. Text chat routing still keeps a legacy `availableModelsByProvider` alias, but media tools resolve from the capability-aware catalog through plan fields `imageGenerateModelKey`, `imageEditModelKey`, and `videoGenerateModelKey`, then materialize into runtime tool credential refs.
