...
- category
- 1-2 tags
- 1-2 short admin-authored examples from the Skill instruction card as semantic routing hints

It does not receive Skill documents.

It does not receive full instruction cards for all enabled Skills in classifier input.
...
| 8    | completed   | User Skills UI and setup/recreate integration                                                                                                      | Setup/recreate exposes optional Skill selection before final preview without auto-enabling; Assistant Settings has a Skills section backed by `GET/PUT /assistant/skills`, plan counters, archived/over-limit disabled states, and no assistant-card summary                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 13   | in_progress | End-to-end verification and founder validation support                                                                                             | Focused automated tests pass and a concise founder smoke script exists for manual live validation. Base Skills are available through a repeatable admin-curated seed, Skill routing uses sticky per-chat auto state, and admin-owned Product/Skill KB retrieval now has separate `/admin/knowledge` embedding/helper model slots so user KB can remain plan-owned. The web SSE parser now preserves Skill activity metadata (`skillName`, `skillIconEmoji`) through the real stream path, and the web activity merger carries the compact `Навык - <emoji>` marker forward across later retrieval/tool/runtime banners so Skill visibility survives non-`Ответ готов` final activity states. Final live smoke still needs the enabled Skill/document-ready path checked after deploy. |

Current next step:

1. Continue Step 13: deploy the Skill routing optimization and admin-owned KB retrieval policy, reindex Product/Skill documents with the configured embedding model, then run founder live smoke for enabled Skill retrieval.
