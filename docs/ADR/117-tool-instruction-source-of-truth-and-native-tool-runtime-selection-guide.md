...
| **HOW the provider renders** (anti-collage, standalone-image, series-cardinality, reference-guidance)                    | **Provider-conditioning fragments**     | `packages/runtime-contract/src/index.ts` (canonical fragments), consumed by runtime media composers and provider-gateway builders only |
...
### Post-Slice-5 hotfix (2026-06-15) — provider fragments folded into the contract index

Slice 3 originally placed the provider-conditioning fragments in a sibling file `packages/runtime-contract/src/media-prompt-fragments.ts`, re-exported from `index.ts`. That re-export was the **first relative import** ever added to the contract package, and it broke production startup: `@persai/runtime-contract` is consumed as **un-built TypeScript source** at runtime (`main` → `src/index.ts`, no build step), and Node 22's type-stripping ESM loader cannot resolve an extensionless relative specifier (`ERR_MODULE_NOT_FOUND`), crash-looping `api` and `runtime` on the rollout image. `.ts`-extension imports fail the emit typecheck (TS5097), and there is no build emitting a `.js` sibling. Fix: the fragments were **inlined directly into `src/index.ts`** and the sibling file deleted, restoring the single self-contained module. Single-source ownership is unchanged — the canonical definitions now live in the contract index module, still consumed by all media composers/builders via `@persai/runtime-contract`. The golden single-source test now anchors on `index.ts`. If the package ever gains a real build step (like `@persai/config`/`logger`/`types`), the fragments may be re-split.
