...
- **Slice 4 follow-up (live correctness) — landed locally, live re-validation pending.** Live `persai-dev` use surfaced two DeepSeek-specific behaviours the thin adapter must honour, both diagnosed against the cluster + DeepSeek's official API docs (not guessed):
  - **Thinking-mode `reasoning_content` round-trip.** DeepSeek V4 is in thinking mode by default; a tool-loop continuation that omits the `reasoning_content` produced with the prior `tool_calls` is rejected `400 "The reasoning_content in the thinking mode must be passed back to the API."`. The adapter now captures `reasoning_content` (stream `delta.reasoning_content` + non-stream `message.reasoning_content`), carries it on `ProviderGatewayTextGenerateResult.reasoningContent` and `ProviderGatewayToolExchange.reasoningContent`, echoes it on the assistant tool-call message, and forces `parallel_tool_calls: false` so each assistant turn carries exactly one tool call and the echo is canonical. OpenAI/Anthropic ignore the field. This is provider-native behaviour, not a routing change.
  - **Text-only multimodal degradation.** DeepSeek's chat API is text-only; routing image/PDF blocks to it 400s. A runtime `runtime-text-only-multimodal-sanitizer` converts non-text blocks to text for text-only main providers by describing each via the plan's `systemTool` slot model (vision-capable OpenAI/Anthropic) at two chokepoints (base messages once before the tool loop; `files.preview` follow-up blocks when read). Vision-capable main providers are untouched; an unavailable helper or failed describe yields an explicit placeholder rather than raw pixels or a silent drop. The admin plan validator now requires the `systemTool` slot to be vision-capable, since that slot is the describe helper.
- **Verification — green locally.** Full AGENTS gate plus ADR-124 focused tests/typechecks pass. Deploy-time validation remains required for `gpt-5.5` prompt-cache retention, Anthropic numeric-schema sanitation, pre-first-token provider-failure fallback, mixed-provider slot selection, DeepSeek credential-gated chat/structured-output/fallback smoke, **DeepSeek tool-loop turns (reasoning_content round-trip), and DeepSeek turns with image/PDF/document attachments (systemTool describe path)**.
