...
- **2026-07-16 — mid-work bridge reconnect + device-token TTL (local).** Founder chrome://extensions Errors showed `wss://api.persai.dev/.../browser-bridge/ws` `net::ERR_CONNECTION_RESET` while the assistant was actively using the desktop bridge. Two defects stacked: (1) the Web Store dial-spam budget counted `onerror` after a socket that had already reached OPEN, so a handful of LB flaps paused reconnect until fresh registration; (2) device tokens still expired at 15 minutes while long connected work kept the socket up — after a late reset the extension refused to redial past the 14-minute local safe-age wall. Fix: count consecutive failures only for pre-open dials and reset backoff after a healthy open; extend API device-token TTL to 4h with matching client safe-age; extension status reports `registrationUpdatedAt`; app-shell maintainer renews credentials for a still-connected matching scope after 3h. Rebuild/reload the unpacked extension after deploy. No new ADR.
