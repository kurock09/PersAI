...
| Current active step                | in_progress | `Step 15 — Introduce bounded inline tools and async worker jobs` remains active; `T15-0`, `T15-1`, `T15-2`, `T15-3a — Shared native tool runtime hardening and system helpers`, `T15-3b — Web search and fetch plan tools`, and `T15-4 — Browser and web-interaction plan tools` are complete, and `T15-5 — Reminder and scheduled action plan tools` is now in progress with the worker-contract baseline, the first real native reminder projection/executor seam, and a shared PersAI-owned delivery/fanout core landed. `reminder_task` now compiles as a worker `scheduled_action`, executes on the native tool loop through PersAI internal task registry/control endpoints, and the current reminder/task UI path is code-ready for bounded live validation while hidden internal `cron` stays only as temporary scheduler machinery. PersAI-owned due-time pickup remains the next follow-through. `persai_workspace_attach` / `persai_tool_quota_status` stay non-model-visible until later `T15-7` exposure/executor follow-through                                                                                                                                                                                                                                                                                                             |
...
| T15-5 — Reminder and scheduled action plan tools                 | in_progress | Step 15  | `reminder_task` stays the product-facing surface while PersAI-native scheduler/worker triggers remain hidden internal machinery instead of carrying forward raw legacy `cron` semantics. The first three sub-steps are now landed: `reminder_task` compiles as a worker `scheduled_action` on the shared `runtime.workerTools` layer, native runtime now projects/executes the product-facing `create` / `list` / `pause` / `resume` / `cancel` reminder surface through PersAI internal task registry/control seams plus clean native `conversationContext` for Telegram target routing, and current reminder delivery/fanout now runs through one shared PersAI-owned delivery core instead of living only inside the legacy `cron-fire` adapter. Hidden internal `cron` stays internal-only as the temporary due-time trigger source; PersAI-owned due-time pickup remains the later follow-through. This leaves the current reminder/task UI path code-ready for bounded live validation without reopening raw scheduler semantics on the model/product surface                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
