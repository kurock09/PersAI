...
| Runtime replacement implementation | in_progress | Steps 1-14 are complete, and Step 15 now has `T15-0` through `T15-6` landed on the PersAI-native runtime path. Native web retrieval, browser, reminders/scheduled actions, `image_generate`, `image_edit`, `tts`, and `video_generate` now run through the shared runtime/provider-gateway/object-storage boundary, while `persai_workspace_attach` / `persai_tool_quota_status` remain intentionally dark until later `T15-7` follow-through. The next queued Step 15 family is `T15-6b — Human memory and namespaced knowledge sources`. |
...
| Current active step                | in_progress | `Step 15 — Introduce bounded inline tools and async worker jobs` remains active. `T15-6 — Media generation and editing plan tools` is now complete on the shared native media boundary (`image_generate`, `image_edit`, `tts`, `video_generate`, truthful web/Telegram media status UX, reference-image video generation, timeout alignment, and per-plan `video_generate` model selection), and the next active Step 15 family is `T15-6b — Human memory and namespaced knowledge sources`. |
...
| Step 15 — Introduce bounded inline tools and async worker jobs | in_progress | Steps 9-10 | `T15-0` through `T15-6` are now landed. The native Step 15 runtime now covers shared compaction, web retrieval, browser, scheduled actions/reminders, and the full first media family (`image_generate`, `image_edit`, `tts`, `video_generate`) on one shared worker/provider-gateway/object-storage/media-delivery boundary, including truthful web/Telegram media status UX, bounded prompt-plus-reference-image video generation, worker-derived timeout alignment, and admin-managed per-plan `video_generate` model selection (`sora-2` / `sora-2-pro`). `T15-6b` is the next queued follow-through, and later helper exposure work stays pending in `T15-7`. |
