...
| Current active step                | in_progress | `Step 15 — Introduce bounded inline tools and async worker jobs` remains active; `T15-0`, `T15-1`, `T15-2`, `T15-3a`, `T15-3b`, `T15-4`, and `T15-5` are complete, and `T15-6 — Media generation and editing plan tools` is still the current slice. Native `image_generate`, native `image_edit`, and native `tts` are now all real landed media workers on the shared PersAI-owned worker/provider-gateway/object-storage/media-delivery path. `image_edit` now uses the shared image credential seam plus a numbered current-turn source image and optional second current-turn reference image for prompt-guided edits across web chat and Telegram, with explicit edit-intent guidance so ordinary image-analysis turns stay on the normal multimodal path and common two-image reference-guided requests such as "like the second photo" can resolve onto `source=image #1` plus `reference=image #2` without requiring manual role UI. `video_generate` remains the next additive follow-through inside `T15-6`, `T15-6b` stays separate, and `persai_workspace_attach` / `persai_tool_quota_status` remain later `T15-7` exposure work |
...
| T15-6 — Media generation and editing plan tools | in_progress | Step 15 | `T15-6` now has three honest landed media workers on the shared native boundary: `image_generate`, `image_edit`, and `tts`. `image_edit` is now end-to-end real with a typed native contract, shared `tool_image_generate` credential reuse, provider-gateway `edit-image` execution over OpenAI `images.edit`, runtime worker projection/execution that edits a numbered current-turn source image with an optional second current-turn reference image, explicit model guidance to invoke only for real edit intent, a conservative common-case fallback for prompts like "like the second photo" / `как на втором фото`, strengthened provider prompt guidance that tells OpenAI to edit only the source image while treating the second image as a visual reference, PersAI object-storage artifact persistence, and the same API/native media delivery support already used by web chat and Telegram. `video_generate` remains the next additive follow-through inside the slice rather than being faked behind a premature generic media abstraction |
