...
| Current active step | in_progress | Step 10 — Add web shadow comparison and cut over web now includes explicit sync/stream web route modes, bounded native shadow-comparison logging, a pod-local Admin Overview read model for recent parity samples, and recent canonical web chat history hydration inside `apps/runtime` |
