...
| Runtime replacement implementation | in_progress | Steps 1-14 are complete. PersAI now owns Telegram webhook ingress/delivery, group/chat metadata sync, canonical Telegram transcript persistence, and native request-time text/group execution. Step 15 is now active with `T15-0`, `T15-1`, `T15-2`, and `T15-3a` complete: shared compaction now has a typed native bundle contract, dark native runtime seams, public web manual/state/banner routing on native truth, Telegram auto summarize through the same runtime-owned compaction service, rolling-latency web banner signaling, later-turn summary reuse, validated `persai.runtimeSessionCompaction.v2` durable-state semantics with bad-row read guards, and a hardened shared tool runtime that exposes only real compaction tools, streams real OpenAI/Anthropic `tool_calls` with resumed post-tool output, emits explicit tool lifecycle events, classifies internal follow-up/compaction requests honestly, and reuses the accepted-turn lease for in-turn compaction without redundant same-turn auto-compaction. `T15-3b` is now in progress with native `web_fetch` landed end-to-end and native `web_search` now honoring the full current control-plane provider set (`Tavily`, `Brave`, `Perplexity`, `Google (Gemini)`) on the shared native `query` / `count` contract: runtime projects it only when inline policy + configured credential truth agree and the selected provider is known, consumes the daily limit through the internal API, and provider-gateway resolves the same tool credential plus returns normalized untrusted search results across all current provider seams. Bounded post-`reapply` live validation is the next honest follow-through, and `persai_workspace_attach` / `persai_tool_quota_status` remain intentionally dark until later `T15-7` exposure/executor follow-through |
...
| Current active step | in_progress | `Step 15 — Introduce bounded inline tools and async worker jobs` remains active; `T15-0`, `T15-1`, `T15-2`, and `T15-3a — Shared native tool runtime hardening and system helpers` are now complete. `T15-3b — Web search and fetch plan tools` is now the active tool slice; native `web_fetch` is landed through the shared tool loop + provider-gateway Firecrawl seam, native `web_search` now covers the full current admin-visible provider set, bounded live validation from a freshly applied bundle is next, and `persai_workspace_attach` / `persai_tool_quota_status` stay non-model-visible until later `T15-7` exposure/executor follow-through |
...
| Step 15 — Introduce bounded inline tools and async worker jobs | in_progress | Steps 9-10 | `T15-0` now freezes the current catalog/UI/runtime tool surface, `T15-1` adds explicit native `toolPolicies` metadata (`system|plan|internal`, `allowed|forbidden`, `inline|worker|sandbox`) plus prompt/runtime parity, `T15-2` completes the typed `runtime.sharedCompaction` contract plus native compaction/session-resolve seams and caller cutovers with validated durable-state reuse and bad-row guards, and `T15-3a` now closes the shared runtime hardening pass: sync/stream turns expose only real compaction tools, OpenAI/Anthropic streaming can stop on `tool_calls` and continue after tool results, compaction tool calls reuse the accepted-turn lease, internal request classification is explicit, same-turn durable compaction suppresses redundant post-turn auto-compaction, and helper tools without PersAI-native executors stay dark. `T15-3b` is now underway with native `web_fetch` landed end-to-end on the shared tool loop using runtime-owned daily-limit consumption plus provider-gateway Firecrawl fetch normalization/untrusted attribution, and native `web_search` now landed across the full current provider surface instead of a `Tavily`-only subset. Bounded live validation plus later helper exposure/admin follow-through remain pending in `T15-3b` / `T15-7` |
