...
- **Landing rule**
  - open `T15-6` with the smallest honest worker baseline: land `image_generate` end to end first through native provider-gateway routing, runtime artifact persistence, and API media delivery, while `tts`, `image_edit`, and `video_generate` remain later additive follow-through inside the same slice
- **Included**
