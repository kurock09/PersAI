...
| Current active step                | in_progress | `Step 15 — Introduce bounded inline tools and async worker jobs` remains active; `T15-0`, `T15-1`, `T15-2`, `T15-3a`, `T15-3b`, `T15-4`, and `T15-5` are complete, and `T15-6 — Media generation and editing plan tools` is still the current slice. Native `image_generate`, native `image_edit`, `tts`, and `video_generate` are now all real landed media workers on the shared PersAI-owned worker/provider-gateway/object-storage/media-delivery path. `image_edit` uses the shared image credential seam plus a numbered current-turn source image and optional second current-turn reference image for prompt-guided edits across web chat and Telegram, while `video_generate` now lands two bounded native flows on the same seam: prompt-only generation and prompt plus one current-turn reference image. `T15-6b` stays separate, and `persai_workspace_attach` / `persai_tool_quota_status` remain later `T15-7` exposure work |
...
| Step 15 — Introduce bounded inline tools and async worker jobs | in_progress | Steps 9-10 | `T15-0` now freezes the current catalog/UI/runtime tool surface, `T15-1` adds explicit native `toolPolicies` metadata, `T15-2` completes the typed `runtime.sharedCompaction` contract plus native compaction/session-resolve cutovers, `T15-3a` closes the shared runtime hardening pass, `T15-3b` lands real native web retrieval, `T15-4` lands the first real native browser path, `T15-5` lands the scheduled-action worker-contract and PersAI-owned reminder/scheduler follow-through, and `T15-6` remains active on the honest native media baseline. Native `image_generate`, native `image_edit`, `tts`, and `video_generate` now all run through the shared worker/provider-gateway/object-storage/media-delivery path, with `image_edit` adding numbered current-turn source/reference image selection plus explicit edit-intent guidance and `video_generate` adding a typed native contract, `generate-video` provider-gateway seam, shared `tool_image_generate` credential reuse, bounded prompt-only and prompt-plus-reference-image flows, longer worker timeout handling, and persisted video artifact delivery across web chat and Telegram. `T15-6b` remains separate, and later helper exposure work stays pending in `T15-7` |
...
| T15-6 — Media generation and editing plan tools | in_progress | Step 15 | `T15-6` now has four honest landed media workers on the shared native boundary: `image_generate`, `image_edit`, `tts`, and `video_generate`. `image_edit` is end-to-end real with a typed native contract, shared `tool_image_generate` credential reuse, provider-gateway `edit-image` execution over OpenAI `images.edit`, runtime worker projection/execution that edits a numbered current-turn source image with an optional second current-turn reference image, explicit model guidance to invoke only for real edit intent, a conservative common-case fallback for prompts like "like the second photo" / `как на втором фото`, strengthened provider prompt guidance that tells OpenAI to edit only the source image while treating the second image as a visual reference, PersAI object-storage artifact persistence, and the same API/native media delivery support already used by web chat and Telegram. `video_generate` is now also end-to-end real on that same boundary with a typed native contract, shared `tool_image_generate` credential reuse, provider-gateway `generate-video` execution over bounded OpenAI `sora-2` create/poll/download flow, runtime worker projection/execution for both prompt-only and prompt-plus-current-turn-reference-image scenarios, longer worker timeout handling, and persisted video artifact delivery rather than a fake placeholder abstraction |
