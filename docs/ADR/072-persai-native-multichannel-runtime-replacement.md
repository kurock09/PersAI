...
| `tts`                      | `plan_managed`, cost-driving, visible in the plan editor; credential seams `tool_tts_openai`, `tool_tts_elevenlabs`, and `tool_tts_yandex` plus admin primary-provider selection and assistant-scoped voice profile | Preserve as the explicit Step 15 `tts` plan tool in `T15-6`; land it through the shared worker/runtime/provider-gateway media path with provider fallback and persona-aware voice steering, and keep `Step 15a` channel voice output separate from tool-driven TTS semantics                                                                                                                                          |
