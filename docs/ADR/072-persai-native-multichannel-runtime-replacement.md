...
- if API-side quota degrade routing already exists, native `createTurn` must preserve it explicitly instead of dropping provider/model override semantics at the cutover boundary
...
| Runtime replacement implementation | in_progress | Steps 1-8 are complete, and Step 9 is now in progress: `apps/provider-gateway` has the first text-generation seam, `apps/runtime` has a dark sync text-only `createTurn` path over the Step 8 session-state package, and `apps/api` can now route sync web turns to that native path behind `PERSAI_NATIVE_RUNTIME_WEB_SYNC_ENABLED`. The sync path has already been rolled out once through the normal dev GitOps flow and passed one bounded live smoke on dev GKE, but `streamTurn`, primary web UX cutover, and final removal of the Step 7 activation seams still remain |
...
| Current active step | in_progress | Step 9 — Implement native web `createTurn` and `streamTurn` now has both a dark sync `createTurn` execution seam and a flagged API sync consumer, and that sync path has already passed one bounded dev-GKE live smoke, but the step is not complete yet because `streamTurn` and primary web UX cutover still remain |
...
| Slice 3 — Distributed session/state core and web runtime | in_progress | Web request-time execution runs through native runtime on Redis/Postgres session state |
...
| Step 9 — Implement native web `createTurn` and `streamTurn` | in_progress | Steps 4, 5, 6, 7, 8 | `apps/provider-gateway` now exposes the first text-generation seam, `apps/runtime` now exposes a dark sync text-only `createTurn` path over Step 8 acceptance/finalization, and `apps/api` can route sync web turns to that native path behind `PERSAI_NATIVE_RUNTIME_WEB_SYNC_ENABLED` while keeping canonical replay/message persistence ownership. That sync path has already been rolled out once through normal dev GitOps and passed a bounded live smoke plus replay/idempotency check on dev GKE, but `streamTurn`, primary web UX cutover, and final Step 7 activation-seam removal remain |
