...
- **Landing rule**
  - open `T15-6` with the smallest honest worker baseline: land `image_generate` end to end first through native provider-gateway routing, runtime artifact persistence, and API media delivery, then land explicit `tts` through the same shared worker/runtime/provider-gateway media path with provider fallback, assistant voice profiles, and persisted audio artifacts before moving on to `image_edit` / `video_generate`
- **Included**
...
| Current active step                | in_progress | `Step 15 — Introduce bounded inline tools and async worker jobs` remains active; `T15-0`, `T15-1`, `T15-2`, `T15-3a — Shared native tool runtime hardening and system helpers`, `T15-3b — Web search and fetch plan tools`, `T15-4 — Browser and web-interaction plan tools`, and `T15-5 — Reminder and scheduled action plan tools` are complete, and `T15-6 — Media generation and editing plan tools` is now in progress on the honest native media baseline. Native `image_generate` now has a typed native contract plus provider-gateway `generate-image` seam, runtime worker projection/execution with quota and credential gating, PersAI object-storage artifact persistence, and API-side send/stream/Telegram/media-delivery support for native `RuntimeOutputArtifact`s instead of failing closed when runtime returns media. Native `tts` now also has provider-specific admin credentials with primary-provider selection, deterministic compatibility-aware fallback, assistant-scoped `voiceProfile` draft/publish state, primary-provider-only assistant voice UX with gender-filtered choices, provider-gateway `generate-speech` execution for `ElevenLabs` / `Yandex SpeechKit` / `OpenAI`, runtime worker projection/execution with tone steering plus persisted audio artifacts, and focused provider/runtime/UI coverage that locks the main live root-cause fixes (`ElevenLabs` voice-id gating plus Yandex voice-role compatibility). `image_edit` and `video_generate` remain later follow-through inside `T15-6`, `T15-6b` remains a separate later slice, and `persai_workspace_attach` / `persai_tool_quota_status` stay non-model-visible until later `T15-7` exposure/executor follow-through |
...
| Step 15 — Introduce bounded inline tools and async worker jobs | in_progress | Steps 9-10 | `T15-0` now freezes the current catalog/UI/runtime tool surface, `T15-1` adds explicit native `toolPolicies` metadata for `system`, `plan`, and `internal` tools plus `allowed` / `forbidden` usage and `inline`, `worker`, and `sandbox` execution modes, `T15-2` completes the typed `runtime.sharedCompaction` contract plus native compaction/session-resolve seams and caller cutovers with validated durable-state reuse and bad-row guards, `T15-3a` closes the shared runtime hardening pass, `T15-3b` is complete with real native web retrieval, `T15-4` is complete with the first real native browser path, `T15-5` is complete with the scheduled-action worker-contract baseline plus PersAI-owned reminder/scheduler follow-through, and `T15-6` is now in progress on the honest native media baseline. Native `image_generate` now has a typed native contract, provider-gateway OpenAI execution through the new `generate-image` seam, runtime worker projection/execution with quota and credential gating, PersAI object-storage artifact persistence, and API-side send/stream/Telegram/media-delivery support for native `RuntimeOutputArtifact`s, including the Telegram outbound handoff from transient runtime-output objects onto canonical persisted assistant attachments. `web_fetch` is landed end-to-end on the shared tool loop using runtime-owned daily-limit consumption plus provider-gateway Firecrawl fetch normalization/untrusted attribution, `web_search` is landed across the full current provider surface instead of a `Tavily`-only subset, the native bundle now carries `runtime.workerTools` with enforced parity for every current worker-policy tool (`browser`, `image_generate`, `tts`, `scheduled_action`, hidden internal `cron`), PersAI now has a real `tool_browser` / `Browserless` control-plane seam plus typed `runtime.browser` provider/action/confirmation metadata, runtime/provider-gateway now project and execute the bounded `snapshot` + `act` browser contract through a Browserless-backed worker path with quota/provider gating and structured tool-history results, `scheduled_action` now executes `create` / `list` / `pause` / `resume` / `cancel` through PersAI internal task registry/control seams while due user reminders and assistant background actions are claimed/executed on a PersAI-owned scheduler path instead of hidden legacy cron jobs for new reminders, and native `tts` now has deterministic provider routing, assistant-scoped `voiceProfile` normalization, primary-provider-only voice UX, live ElevenLabs voice selection, and compatibility-aware Yandex/ElevenLabs guards that prevent the main observed live fallback regressions from surviving into request-time execution. `image_edit` and `video_generate` remain later follow-through inside `T15-6`, `T15-6b` remains separate, and later helper exposure work remains pending in `T15-7` |
...
| T15-6 — Media generation and editing plan tools | in_progress | Step 15 | `T15-6` now opens on the smallest honest worker baseline: `image_generate` is landed end to end with a typed native contract, OpenAI-backed provider-gateway routing, runtime worker projection/execution, PersAI object-storage artifact persistence, and API/native media delivery support for `RuntimeOutputArtifact`s. The current follow-through also fixes Telegram outbound delivery to send the canonical persisted assistant attachment copy instead of trying to read the deleted transient runtime-output object. Native `tts` is also now a real landed media worker with provider-specific admin credentials, deterministic compatibility-aware provider routing, assistant-scoped `voiceProfile`, primary-provider-only assistant voice UX with gender-filtered choices plus live ElevenLabs voice selection, Yandex voice-role compatibility guards, persisted audio artifacts, and focused regression coverage that locks the main live stabilization findings. `image_edit` and `video_generate` remain additive follow-through inside the slice rather than being faked behind one premature generic media abstraction |
