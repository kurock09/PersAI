...
- **Included**
  ...
  - the initial worker baseline covers the then-current worker-policy surface (`browser`, `image_generate`, `tts`, hidden internal `cron`) with explicit family/outcome/timeout/confirmation/provider-routing/failure metadata before any one executor family is reactivated; `T15-5` later extends that shared worker layer to `reminder_task`
...
- **Included**
  - `reminder_task`
  - first sub-step remaps `reminder_task` onto the shared `runtime.workerTools` contract as the `scheduled_action` family
  - next sub-step projects and executes the current product-facing `reminder_task` surface (`create`, `list`, `pause`, `resume`, `cancel`) through the native runtime tool loop via PersAI internal task registry/control seams rather than exposing raw scheduler triggers
  ...
  - final sub-step replaces hidden legacy due-time pickup with a PersAI-owned scheduler loop over durable reminder-task rows, a global rollout-resettable scheduler epoch, expiring claims, and retry state while reusing the shared delivery core
...
- **Excluded**
  ...
  - browser/media work
- **Validation**
  - tool-policy/materialization tests prove `reminder_task` now compiles as a worker `scheduled_action` while hidden internal `cron` remains separate internal scheduler machinery
  - native runtime tests prove `reminder_task` now projects and executes through the shared tool loop with structured create/list/pause results
  - API tests prove native reminder create can persist Telegram target routing through clean `conversationContext` input without regressing to raw scheduler exposure
  ...
  - reminder scheduling/execution tests prove new reminders persist durable native schedule/payload state, due reminders are claimed by the hidden PersAI scheduler loop, and recurring reminders compute the next slot without legacy cron jobs
...
- **Goal**
  Land PersAI-native cross-session human memory and namespaced knowledge sources so assistants can remember users between sessions and answer against private/shared corpora without confusing session context, durable human memory, product knowledge, or Step 16 sandbox access, and so ordinary prod turns stop depending on a blunt fixed-count history slice as their primary context strategy. This slice also lands the minimal PersAI-owned storage/ingest/status surface needed to make uploaded knowledge sources real inside ADR-072 rather than deferring all file/source management until later.
- **Included**
...
| Current active step                | in_progress | `Step 15 — Introduce bounded inline tools and async worker jobs` remains active; `T15-0`, `T15-1`, `T15-2`, `T15-3a — Shared native tool runtime hardening and system helpers`, `T15-3b — Web search and fetch plan tools`, and `T15-4 — Browser and web-interaction plan tools` are complete, and `T15-5 — Reminder and scheduled action plan tools` is now code-complete for pre-prod live validation. `reminder_task` now compiles as a worker `scheduled_action`, executes on the native tool loop through PersAI internal task registry/control endpoints, persists native schedule/payload state directly in PersAI task rows, and due reminders are now claimed/executed by a PersAI-owned scheduler loop with a global rollout-resettable scheduler epoch, expiring claims, and retry delay instead of hidden legacy cron jobs. Hidden internal `cron` remains internal-only only as compatibility machinery for old callback rows during rollout, while the active new reminder/task UI path is now ready for bounded live validation on native due-time pickup. `persai_workspace_attach` / `persai_tool_quota_status` stay non-model-visible until later `T15-7` exposure/executor follow-through                                                                                                                                                                                                                                                                 |
...
| Step 15 — Introduce bounded inline tools and async worker jobs | in_progress | Steps 9-10 | `T15-0` now freezes the current catalog/UI/runtime tool surface, `T15-1` adds explicit native `toolPolicies` metadata for `system`, `plan`, and `internal` tools plus `allowed` / `forbidden` usage and `inline`, `worker`, and `sandbox` execution modes, `T15-2` completes the typed `runtime.sharedCompaction` contract plus native compaction/session-resolve seams and caller cutovers with validated durable-state reuse and bad-row guards, `T15-3a` closes the shared runtime hardening pass, `T15-3b` is complete with real native web retrieval, `T15-4` is complete with the first real native browser path, and `T15-5` now has the reminder worker-contract baseline, the real native reminder executor seam, a shared PersAI-owned delivery core for reminder fanout, and a PersAI-owned due-time scheduler loop backed by durable task-row schedule/payload/claim state plus a global reminder scheduler epoch that resets execution claims across rollout/startup. `web_fetch` is landed end-to-end on the shared tool loop using runtime-owned daily-limit consumption plus provider-gateway Firecrawl fetch normalization/untrusted attribution, `web_search` is landed across the full current provider surface instead of a `Tavily`-only subset, the native bundle now carries `runtime.workerTools` with enforced parity for every current worker-policy tool (`browser`, `image_generate`, `tts`, `reminder_task`, hidden internal `cron`), PersAI now has a real `tool_browser` / `Browserless` control-plane seam plus typed `runtime.browser` provider/action/confirmation metadata, runtime/provider-gateway now project and execute the bounded `snapshot` + `act` browser contract through a Browserless-backed worker path with quota/provider gating and structured tool-history results, and `reminder_task` now both compiles as the shared `scheduled_action` worker family and executes `create` / `list` / `pause` / `resume` / `cancel` through PersAI internal task registry/control seams while due reminders are claimed/executed on a PersAI-owned scheduler path instead of hidden legacy cron jobs for new reminders. The current reminder/task UI path is now pre-prod ready for bounded live validation, and later media/helper exposure work remains pending in `T15-6` and `T15-7` |
...
| T15-5 — Reminder and scheduled action plan tools                 | in_progress | Step 15  | `reminder_task` stays the product-facing surface while PersAI-native scheduler/worker triggers remain hidden internal machinery instead of carrying forward raw legacy `cron` semantics. The full code slice is now landed: `reminder_task` compiles as a worker `scheduled_action` on the shared `runtime.workerTools` layer, native runtime now projects/executes the product-facing `create` / `list` / `pause` / `resume` / `cancel` reminder surface through PersAI internal task registry/control seams plus clean native `conversationContext` for Telegram target routing, current reminder delivery/fanout now runs through one shared PersAI-owned delivery core instead of living only inside the legacy `cron-fire` adapter, and new reminders now persist native schedule/payload state directly in PersAI task rows so a hidden PersAI-owned scheduler loop can claim due reminders with a global rollout-resettable scheduler epoch, expiring claims, and retry delay instead of creating legacy cron jobs. Hidden internal `cron` stays internal-only only as legacy callback compatibility for pre-cutover rows during rollout. This leaves the current reminder/task UI path pre-prod ready for bounded live validation without reopening raw scheduler semantics on the model/product surface                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| T15-6b — Human memory and namespaced knowledge sources           | planned   | Step 15  | Cross-session human memory per assistant-user plus preset/subscription, assistant-user-private workspace, and PersAI-global knowledge sources land behind unified `knowledge_search` / `knowledge_fetch`; durable human-memory writes stay explicit tool/validator flows, canonical PersAI-owned storage plus minimal upload/list/delete/status/reindex surfaces land for knowledge files/sources, sandbox remains only an authoring/ingest surface until Step 16, distinct knowledge-storage quota/accounting stays separate from chat media and workspace/sandbox limits, and normal prod context assembly stops treating the current fixed hydrated-history cap as the primary strategy once richer memory/knowledge hydration exists                                                                                                                                                                                                                                                                                                                                                              |
