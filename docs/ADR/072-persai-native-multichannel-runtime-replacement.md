...
| Current active step                | planned     | `Step 15 — Introduce bounded inline tools and async worker jobs` is now complete through `T15-7`. The next unopened work inside `Slice 6` is to choose among `Step 15a — Native web TTS streaming/output`, `Step 15b — Replace bootstrap preset and lifecycle UI with native prompt surfaces`, and `Step 16 — Build isolated sandbox service`. Current inventory `persai_workspace_attach` stays migration-only truth; any later attach-by-ref replacement is post-Step-16 follow-through rather than part of the Step 16 sandbox tool matrix. |
