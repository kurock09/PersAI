...
**2026-04-17 status note:** ADR-072 is now the historical migration ADR through the Step 18 native-path closeout. Active follow-through after that baseline is governed by `docs/ADR/073-post-adr072-residue-and-polish-program.md`, especially for create/recreate polish, cost/quality architecture, Step 19 scale hardening, Step 15a, and Step 20.
...
### 1. The runtime contract still carries temporary compatibility views

PersAI materialization now persists the authoritative native `runtimeBundle` beside neutral `assistantConfig` and `assistantWorkspace` compatibility views in `assistant_materialized_specs`. Those compatibility views are a bounded migration/control-plane seam, not long-term runtime truth, so cleanup still must converge on the native bundle as the only semantic authority.
...
- web keeps a compaction/context-pressure banner when runtime context pressure is crossed and may also show a post-turn auto-compaction success state when the active plan policy enables it
...
Human memory and knowledge access remain part of the ADR-072 target architecture. The first native runtime read backend is now active for assistant-owned uploaded document knowledge, while the broader multi-namespace layer still remains follow-through:

- `knowledge_search` and `knowledge_fetch` are the unified read-contract names inside ADR-072; the first active backends now cover uploaded documents, private memory/chats, shared preset/subscription state, and PersAI-global product knowledge over real PersAI-owned rows/documents
...
- `source="subscription"` stays a bounded read-side knowledge namespace for effective plan/subscription facts, tool-entitlement documents, and similar control-plane truth; it is not the contract for live counters or per-turn remaining usage
...
- `image_edit`
  - shares the current image credential seam with `image_generate` in PersAI control plane
- `tts`
...
| `image_generate`           | `plan_managed`, cost-driving, visible in the plan editor; credential seam `tool_image_generate`                                                                                                                     | Preserve as a first-class Step 15 media-generation plan tool in `T15-6`; later media follow-through must remain additive rather than replacing today's image-generation capability                                                                                                                                                                                                                                                                                                              |
| `image_edit`               | `plan_managed`, cost-driving, visible in the plan editor; shared credential seam `tool_image_generate`                                                                                                              | Land as the adjacent Step 15 prompt-guided image-edit plan tool in `T15-6`, using a numbered current-turn source image plus an optional second current-turn reference image, explicit edit-intent model guidance, conservative "like the second photo" / `как на втором фото` source-vs-reference handling for the common two-image flow, and persisted artifact delivery on the shared native media worker path instead of hiding edits behind `image_generate` or a provider-specific surface |
| `tts`                      | `plan_managed`, cost-driving, visible in the plan editor; credential seams `tool_tts_openai`, `tool_tts_elevenlabs`, and `tool_tts_yandex` plus admin primary-provider selection and assistant-scoped voice profile | Preserve as the explicit Step 15 `tts` plan tool in `T15-6`; land it through the shared worker/runtime/provider-gateway media path with deterministic compatibility-aware fallback, primary-provider-aligned assistant voice UX, and persona-aware voice steering, and keep `Step 15a` channel voice output separate from tool-driven TTS semantics                                                                                                                                             |
...
1. **Bundle under-specification risk**
   If `AssistantRuntimeBundle` omits policy or routing fields that are still duplicated into `assistantConfig` or `assistantWorkspace`, migration will miss business behavior.
...
- no `assistantConfig` or `assistantWorkspace` compatibility-view reads in active request paths
...
- **Included**
  ...
  - web banner that suggests or confirms context compression from runtime context pressure and successful post-turn auto-compaction, rather than from a separate rolling-latency heuristic
  ...
  - first contract baseline is a typed `runtime.sharedCompaction` bundle block sourced from the existing admin compaction policy plus Telegram owner config, carrying the fixed shared tool names and shared compaction execution knobs before executor wiring lands; later `T15-6b` follow-through moves ordinary-turn budget and surface-level auto-compaction policy into plan-editable `runtime.contextHydration`
...
- **Landing rule**
  - open `T15-6` with the smallest honest worker baseline: land `image_generate` end to end first through native provider-gateway routing, runtime artifact persistence, and API media delivery, then keep later `tts`, `image_edit`, and `video_generate` follow-through additive on that same shared worker/runtime/provider-gateway media path instead of splitting media into tool-specific stacks
- **Included**
...
- **Validation**
  ...
  - voice compatibility tests prove stale provider/voice combinations are filtered before materialization and runtime execution
  - UI tests prove assistant voice settings stay aligned to the active primary provider and gender-filtered voice choices
  - admin-plan tests prove `video_generate` model selection stays admin-managed through `billingProviderHints.videoGenerateModelKey`, materializes into the server-owned runtime bundle seam `toolCredentialRefs.video_generate.modelKey`, and only the bounded allowlist `sora-2` / `sora-2-pro` is accepted with fallback to `sora-2`
  - provider/runtime tests prove prompt-plus-reference-image `video_generate` works on the shared native boundary without requiring pre-sized user uploads, because provider-gateway normalizes the selected current-turn reference image before the OpenAI call
  - API/runtime tests prove long-running native media turns use the worker-derived timeout budget instead of failing early on the older 90-second API timeout path
  - web/Telegram UX tests prove media-tool progress stays truthful on the native path: web keeps one live status instead of a growing badge chain, and Telegram maps media tools onto bounded Bot API chat actions during generation/upload
...
- **Included**
  ...
  - a plan-editable `runtime.contextHydration` policy on the server-owned bundle so target budget, compaction trigger, recent-history floor, knowledge-hydration budget, and web/Telegram auto-compaction behavior are tuned per plan rather than hidden in fixed runtime code
  - web/Telegram compaction UX driven by that same policy: web shows context pressure or recent successful auto-compaction, and Telegram may send a short post-reply auto-compaction notice when it actually happens
  - richer retrieval ranking with deterministic lexical ordering, structured field/source boosts, duplicate collapse, and a lightweight local hybrid rerank for the highest-value private namespaces
...
- **Included**
  ...
  - distinct operator/user visibility for token/chat quotas, media upload quotas, and knowledge-source storage quotas from PersAI-owned accounting truth instead of one opaque usage bucket
...
- **Excluded**
  ...
  - folding live quota counters into `knowledge_* source="subscription"`
...
- migrate active request-path reads to native bundle fields before dropping compatibility columns
- ensure repository-wide search returns no live `assistantConfig/assistantWorkspace` request-path use
...
- `infra/helm/*` and active dev GKE deploy/runbook surfaces
- non-UI `apps/api` materialization/runtime helper naming that still leaks OpenClaw terminology
...
- remove `openclaw-*` Helm/GKE runtime workloads once ordinary web and Telegram flows are proven live on the native path
- keep UI-facing contract cleanup and wording cleanup aligned, but non-UI infra/runtime/helper cleanup may land first as a bounded sub-slice
- `System Overview` should prefer a discovered multi-pod aggregate view over manual pod switching: keep trace-source labels explicit, fan the trace toggle out across the discovered pod set, and remove stale pod-local selector copy from the main operator surface
- remove the old `Web Runtime Shadow Compare` block and the stale runtime-tier matrix from the main admin overview once they no longer describe the active native path
...
- Step 19 explicitly owns deploy/restart recovery semantics for the live chat path; routine deploys of `apps/api`, `apps/runtime`, `apps/provider-gateway`, or `apps/web` must not require manual global `reapply all` before ordinary web or Telegram chats work again
- `latest published`, `latest applied/materialized`, and `currently warmed in this runtime pod` must be treated as different states; a cold or replaced runtime pod may need bounded bundle warm recovery, but that must not downgrade the assistant into a fleet-wide `assistant_not_live` gate
- bundle warm loss after pod restart must recover through bounded automatic warm-on-demand, bounded background warming, or another PersAI-owned self-healing path rather than operator-triggered mass reapply
- any bundle/runtime contract migration that truly requires regeneration must be versioned, incremental, and non-blocking for the whole fleet; `reapply all` may remain an emergency operator tool, but it must not be part of the normal deploy path at `10000+` active users
...
- keep `assistantConfig` / `assistantWorkspace` as long-term runtime truth instead of bounded compatibility views beside `runtimeBundle`
...
| Runtime replacement implementation | in_progress | Steps 1-18 are now complete on the main native path. Native web retrieval, browser, reminders/scheduled actions, `image_generate`, `image_edit`, `tts`, and `video_generate` now run through the shared runtime/provider-gateway/object-storage boundary; `apps/api` owns assistant-scoped knowledge-source upload/list/detail/delete/reindex with PersAI-owned storage/chunk rows plus separate knowledge quota accounting; native runtime `knowledge_search` / `knowledge_fetch` now work for uploaded-document `source="document"`, assistant-user-private `source="memory"` / `source="chat"`, shared `source="preset"` / `source="subscription"`, and PersAI-global `source="global"` over real PersAI-owned rows/documents; native runtime also exposes the explicit bounded `memory_write` system tool through a PersAI-owned validator/audit write seam into the assistant memory registry; ordinary turn hydration now preserves bounded durable memory plus reusable compaction before recent-history trimming; Admin Plans now materialize plan-owned `runtime.contextHydration` plus plan-driven web/Telegram compaction UX; `T15-7` is now closed with canonical `quota_status` plus aligned plan/admin/model guidance; and Step 18 closed the remaining active UI/control-plane/runtime tails before scale hardening. |
| Current active slice               | planned     | `Slice 8 — Scale hardening for 10000+ active users` is now the next primary execution area. `Slice 7 — OpenClaw removal and cleanup` is complete on the active production path, and `Step 15a` remains explicitly deferred behind cleanup plus scale hardening.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Current active step                | planned     | `Step 19 — Scale hardening for 10000+ active users` is the next primary production-path step. Step 18 is closed after the pre-UI infra/runtime cleanup, admin overview multi-pod cleanup, admin runtime control cleanup, neutral control-plane naming sweep, and final active-surface wording/schema cleanup.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Slice 7 — OpenClaw removal and cleanup                    | completed | OpenClaw request-time path, schema tails, and active control-plane/UI leakage are removed from PersAI                                                                                                                         |
| Slice 8 — Scale hardening for 10000+ users                | planned   | The PersAI-native runtime path is measured and hardened for sustained multi-pod production load, including deploy/restart recovery without global `reapply all`                                                               |
...
| Step 16 — Remove OpenClaw runtime integration from PersAI active paths           | completed | Steps 10, 12, 14, 15, 15b | The legacy OpenClaw active executor is deleted from `apps/api` production wiring. Web/Telegram active turns now stay on PersAI-native runtime boundaries, active media/session/reset/task/avatar/workspace-memory surfaces were re-homed onto PersAI-owned storage/registry/runtime seams, and the adapter/facade/module wiring plus adapter-only API env/config knobs were removed. The remaining persisted compatibility naming then moved into Step 17, while broader wording/control-plane cleanup remains Step 18.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Step 17 — Remove OpenClaw-shaped schema and document cleanup                     | completed | Step 16                   | `assistant_materialized_specs` now uses neutral persisted compatibility fields (`assistant_config`, `assistant_workspace`, `assistant_config_document`, `assistant_workspace_document`), domain/repository/service code now uses `assistantConfig` / `assistantWorkspace`, active request-path runtime model override reads `runtimeBundle` instead of compatibility materialization, and the internal runtime freshness response now returns refreshed `assistantConfig` / `assistantWorkspace` payloads.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Step 18 — Audit and clean UI/control-plane/runtime tails                         | completed | Step 17                   | Active admin/setup/runtime surfaces were cleaned of the remaining OpenClaw/bootstrap leakage. Helm/runtime helper cleanup is landed, `System Overview` now uses a discovered multi-pod native-runtime aggregate instead of the old pod-local selector + runtime-tier semantics, `/admin/runtime` keeps only live provider/model/key controls, active provider-settings mode naming is neutralized to `unconfigured_default`, old `openclaw-*` helper/test file paths were replaced with neutral assistant/channel/capability naming, and repo-truth Prisma/control-plane schema no longer carries the dead `optimization_policy` field.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Step 19 — Scale hardening for 10000+ active users                                | planned   | Step 18                   | The primary PersAI-native path is proven and optimized for sustained production concurrency, and routine deploy/restart no longer forces fleet-wide `reapply all` before chats become live again                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| T15-2 — Shared summarization and compaction tools                | completed | Step 15  | `runtime.sharedCompaction` now materializes the shared `summarize_context` / `compact_context` naming plus shared compaction execution thresholds and Telegram auto-summarize policy, `apps/runtime` now has dark native `POST /api/v1/turns/compact` and `POST /api/v1/turns/session/resolve` seams for shared compaction execution and session-state reads, public web manual compaction plus GET/banner state call those seams, Telegram owner auto summarize now reuses the same runtime-owned compaction path after native turns, later `T15-6b` follow-through moves ordinary-turn budget and banner/no-notice semantics onto plan-owned `runtime.contextHydration` plus explicit post-turn `autoCompaction` outcomes, later runtime turns reuse only validated durable compaction state, and bad/legacy compaction rows are ignored safely on the read path                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
