...
- **Included**
  ...
  - the initial worker baseline covers the current worker-policy surface (`browser`, `image_generate`, `tts`, hidden internal `cron`) with explicit family/outcome/timeout/confirmation/provider-routing/failure metadata before any one executor family is reactivated
...
| Runtime replacement implementation | in_progress | Steps 1-14 are complete. PersAI now owns Telegram webhook ingress/delivery, group/chat metadata sync, canonical Telegram transcript persistence, and native request-time text/group execution. Step 15 is now active with `T15-0`, `T15-1`, `T15-2`, `T15-3a`, `T15-3b`, and `T15-4` complete: shared compaction now has a typed native bundle contract, dark native runtime seams, public web manual/state/banner routing on native truth, Telegram auto summarize through the same runtime-owned compaction service, rolling-latency web banner signaling, later-turn summary reuse, validated `persai.runtimeSessionCompaction.v2` durable-state semantics with bad-row read guards, a hardened shared tool runtime that exposes only real compaction tools, real native web retrieval executors, a shared `runtime.workerTools` block for the current worker-policy surface, and a real Browserless-backed native browser path with `tool_browser`, typed `runtime.browser`, provider-gateway browser execution, and runtime worker-tool projection/execution for the bounded `snapshot | act` browser contract. Native `web_search` now honors the full current control-plane provider set (`Tavily`, `Brave`, `Perplexity`, `Google (Gemini)`), native `web_fetch` executes through the Firecrawl seam when its credential is configured, and focused runtime/provider-gateway tests now cover the native browser path as well. `persai_workspace_attach` / `persai_tool_quota_status` remain intentionally dark until later `T15-7` exposure/executor follow-through, while the next Step 15 follow-through moves to `T15-5` |
...
| Step 15 — Introduce bounded inline tools and async worker jobs | in_progress | Steps 9-10 | `T15-0` now freezes the current catalog/UI/runtime tool surface, `T15-1` adds explicit native `toolPolicies` metadata (`system|plan|internal`, `allowed|forbidden`, `inline|worker|sandbox`) plus prompt/runtime parity, `T15-2` completes the typed `runtime.sharedCompaction` contract plus native compaction/session-resolve seams and caller cutovers with validated durable-state reuse and bad-row guards, `T15-3a` closes the shared runtime hardening pass, `T15-3b` is complete with real native web retrieval, and `T15-4` is now complete with the first real native browser path. `web_fetch` is landed end-to-end on the shared tool loop using runtime-owned daily-limit consumption plus provider-gateway Firecrawl fetch normalization/untrusted attribution, `web_search` is landed across the full current provider surface instead of a `Tavily`-only subset, the native bundle now carries `runtime.workerTools` with enforced parity for every current worker-policy tool (`browser`, `image_generate`, `tts`, hidden internal `cron`), PersAI now has a real `tool_browser` / `Browserless` control-plane seam plus typed `runtime.browser` provider/action/confirmation metadata, and runtime/provider-gateway now project and execute the bounded `snapshot | act` browser contract through a Browserless-backed worker path with quota/provider gating and structured tool-history results. Later reminder/media/helper exposure follow-through still remain pending in `T15-5`, `T15-6`, and `T15-7` |
