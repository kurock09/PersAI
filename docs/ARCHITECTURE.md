...
- assistant-sent email (ADR-168) is a **second, separate** outbound path beside the notification platform, not an extension of it: a workspace verifies its own sending address through the Postmark Sender Signatures API (Account token, bounded demand-driven confirmation re-check, surfaced as the fourth card in Settings → Интеграции), and the model-visible `email_send` tool sends synchronously in-turn through an internal API endpoint using the Server Token. Without a verified workspace address no Postmark call happens and the tool returns the standard `skipped` + guidance contract; there is no PersAI-domain fallback sender. Daily/per-turn limits reuse the shared tool-quota mechanism, every attempt writes an `AssistantAuditEvent`, and both this path and `EmailChannelAdapter` share one Postmark send transport while notification policies, quiet hours, and dedupe stay untouched.
