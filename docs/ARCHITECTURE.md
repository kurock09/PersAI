...
- unified notification platform (ADR-088 Slices 1+2+3 implemented): `NotificationIntentService` is the single entry point for all notification creation; all conversational producers (`PersaiIdleReengagementSchedulerService`, `PersaiBackgroundTaskSchedulerService`, `HandleInternalCronFireService`, `QuotaAdvisoryFollowUpService`) and billing lifecycle producer (`BillingLifecycleProducerService`) create intents; `NotificationDeliveryWorkerService` claims/renders/delivers via typed channel adapters; `Admin > Notifications` is the canonical operator control plane for policy, routing, channel health, delivery history, and dead letters; legacy `assistant_notification_outbox`, `assistant_quota_advisory_states`, `workspace_notification_policies` (idle + quota), and `billing_lifecycle_notification_jobs` tables deleted; Slice 3 delivers real billing email via Postmark (`notifications.persai.dev`, six template modules, `class=transactional`, `renderStrategy=template`, dedupeKey, traceId=billing event id)
- PersAI-owned billing lifecycle state, trusted provider/admin billing event snapshots, append-only lifecycle events, admin-owned billing lifecycle notification policy, and durable billing notification jobs (now creating `notification_intents`); billing email delivery pending Slice 3
