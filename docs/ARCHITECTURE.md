...
- document source extraction is API-owned shared infrastructure: Knowledge indexing and document workflows both use the API `DocumentExtractionService` for local text/PDF/DOCX extraction, provider-backed OCR/parsing, provider trace, and quality metadata. Runtime receives pre-extracted transient `sourceFiles[]` only for the deferred PDF/presentation worker lanes and must not duplicate Knowledge OCR/provider-selection logic.
