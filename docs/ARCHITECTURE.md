...
- assistant-sent email (ADR-168, mailbox transport superseded by ADR-169) is a **second, separate** outbound path beside the notification platform, not an extension of it: a workspace connects its own Mail.ru or Yandex mailbox over OAuth (surfaced as the fourth card in Settings → Интеграции), and the model-visible `email_send` tool sends synchronously in-turn through an internal API endpoint over that mailbox's own SMTP with XOAUTH2 — never through Postmark. Without a connected mailbox no SMTP call happens and the tool returns the standard `skipped` + guidance contract; there is no PersAI-domain fallback sender. Daily/per-turn limits reuse the shared tool-quota mechanism, every attempt writes an `AssistantAuditEvent`, and `EmailChannelAdapter`/`PostmarkEmailSendClientService` keep Postmark exclusively for ADR-088 platform notifications while notification policies, quiet hours, and dedupe stay untouched. ADR-169 S5 deleted the ADR-168 Postmark Sender Signatures verification layer this path originally used.
...
- ADR-167 makes that ordinary-turn identity singular across API replicas:
  ...
  share one process stream (live: after streamed answer text / before cursor;
  after commit: folded into the «Выполнено» expand). The terminal attachment
  strip below the answer owns the files. `inlineMediaPlacement` remains an
  F5/history order hint.
...
**ADR-135 catalog tool projection (closed; wire-expansion rule narrowly superseded by ADR-161):** plan-visible tools project as either **full** (complete description + schema every turn) or **catalog** (short `modelDescription` + load hint; full contract via `{toolCode}({ action: "describe" })` on the same tool). Platform defaults seed **13 full / 11 catalog**; admin plan editor exposes one **Full JSON on wire** checkbox per tool (`fullProjection`). A successful describe returns the full description/schema only in its immutable tool result and records a turn-local loaded contract; `tools[]` stays on the original plan-selected catalog projection for the entire turn on every provider. Real execution without prior describe returns structured `tool_contract_not_loaded`; after describe, the catalog stub accepts returned-contract arguments while Runtime validates and authorizes the real call server-side. Runtime logs per turn: `tools_json_char_count`, `catalog_describe_calls`, `tool_contract_not_loaded`. Media-job worker paths keep full persisted request shapes (projection-only savings).

**ADR-164 tool observation spill (open):** mid-loop provider `toolHistory` appends every tool-call ↔ result pair, but oversized bodies (args/results over the wire soft max) spill to `sessions/<sessionId>/.tool-spill/` and the model sees a short receipt (path + summary). Spill paths are model-addressable and hidden from user Working Files / list / search (ADR-150 install-layer spirit). Amends ADR-161 “append full” wire meaning; does not reopen ADR-143/156 mid-loop dual windows. See `docs/ADR/164-tool-observation-spill-and-short-receipts.md`.
