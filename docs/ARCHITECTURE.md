...
- tool-generated media (`image_generate`, `image_edit`, `tts`) is captured from native runtime response payloads and persisted as `tool_output` attachments after turn completion; `image_edit` now edits a numbered current-turn source image with an optional second current-turn reference image, only on explicit edit intent, and outbound delivery stays on the shared PersAI media-delivery path with natural model status text during generation
