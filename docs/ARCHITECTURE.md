...
- tool-generated media (`image_generate`, `image_edit`, `video_generate`, `tts`) is captured from native runtime response payloads and persisted as `tool_output` attachments after turn completion; `image_edit` edits a numbered current-turn source image with an optional second current-turn reference image only on explicit edit intent, `video_generate` now supports bounded prompt-only and prompt-plus-current-turn-reference-image flows on that same native media boundary, and outbound delivery stays on the shared PersAI media-delivery path with natural model status text during generation
- plan-controlled `video_generate` model selection is admin-owned control-plane state, not a model-facing tool argument: `billingProviderHints.videoGenerateModelKey` materializes onto the native runtime bundle seam `toolCredentialRefs.video_generate.modelKey`, accepts only the bounded allowlist `sora-2` / `sora-2-pro`, and defaults back to `sora-2` when unset
