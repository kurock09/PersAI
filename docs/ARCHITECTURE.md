...
- unified notification platform (ADR-088 Slices 1+2 implemented): `NotificationIntentService` is the single entry point for all notification creation; all conversational producers (`PersaiIdleReengagementSchedulerService`, `PersaiBackgroundTaskSchedulerService`, `HandleInternalCronFireService`, `QuotaAdvisoryFollowUpService`) and billing lifecycle producer (`ScheduleBillingLifecycleNotificationsService`) create intents; `NotificationDeliveryWorkerService` claims/renders/delivers via typed channel adapters; `Admin > Notifications` is the canonical operator control plane for policy, routing, channel health, delivery history, and dead letters; legacy `assistant_notification_outbox`, `assistant_quota_advisory_states`, and `workspace_notification_policies` (idle + quota) tables deleted
