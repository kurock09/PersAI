...
- T15-6 media-generation/editing closeout validates:
  ...
  - `video_generate` is now also a first-class `plan_managed` worker tool in catalog/policy/runtime truth, reuses that same `tool_image_generate` credential seam, and extends the shared worker baseline with a longer bounded timeout rather than introducing a separate media stack
  ...
  - runtime projects `video_generate` only when worker policy plus the shared media credential are both active, and the model-visible contract stays bounded to two honest flows: prompt-only generation and prompt plus one current-turn reference image
  - admin plan API/UI now carries `videoGenerateModelKey` with the bounded allowlist `sora-2 | sora-2-pro | null`, materializes that value into the native runtime bundle seam `toolCredentialRefs.video_generate.modelKey`, and never exposes arbitrary provider model ids to the model-facing tool contract
  - common two-image reference-guided prompts such as "like the second photo" / "как на втором фото" can now conservatively resolve to `image #1` as source plus `image #2` as reference when the request is otherwise clear, while truly ambiguous multi-image edit requests still skip honestly instead of guessing
  ...
  - runtime consumes the `video_generate` daily limit before provider execution, forwards prompt-only or prompt-plus-reference-image requests through `POST /api/v1/providers/generate-video`, and persists generated video `RuntimeOutputArtifact`s back onto the same shared media-delivery path used by web chat and Telegram
  - provider-gateway validates the shared image-edit request contract, resolves the same PersAI image secret as `image_generate`, and maps OpenAI `images.edit` onto the native provider result contract with normalized artifact bytes/prompt/usage metadata while strengthening source-vs-reference prompt guidance for two-image reference-guided edits
  - provider-gateway also validates the native video-generation request contract, resolves the same shared media secret, normalizes current-turn reference images before the OpenAI upload, and maps bounded OpenAI `sora-2` / `sora-2-pro` create/poll/download behavior onto the native video result contract with safe fallback to `sora-2`
