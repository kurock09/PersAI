...
13. Slice 6 admin readiness coverage: `test/manage-admin-runtime-provider-settings.service.test.ts` must prove `updateLiveVoiceReadiness` performs a focused single-column update with no materialization rollout queued and no config-generation bump (readiness is read fresh on every session start). The `Admin -> Tools -> Live Voice` readiness controls (enable toggle, Agent ID, direct/relay route) load via `GET /api/v1/admin/runtime/live-voice` and save via step-up-gated `PUT`, without touching the general provider-settings replace flow.
14. Slice 6 relay ingress: `helm template ... --show-only templates/ingress.yaml` must show `/api/v1/assistant/live-voice/relay` on host `persai.dev` routed to the `api` backend, with the `/` catch-all still routed to `web`.
15. Slice 6 live UX rework: the live entry stays accessible (RTL `getByRole`) when enabled and the composer is empty, remains hidden while streaming/recording/disabled, and must not regress the hold-to-record voice-note gesture; the indicator renders compact non-blocking states (connecting/listening/speaking/recovering) plus auto-dismissing error/unavailable pills.
