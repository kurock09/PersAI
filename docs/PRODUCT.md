...
Provider keys are a platform-admin operational concern, not an end-user preference surface.

For the current product direction:

- runtime provider keys (`OpenAI`, `Anthropic`) are managed as one global platform setting, not per user
- primary/fallback model defaults are also platform-admin controlled
- available model catalogs are stored centrally first, then later filtered by plans/entitlements
- raw keys are write-only inputs and must never be shown back in normal product read models
