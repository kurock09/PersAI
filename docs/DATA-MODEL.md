...
- first-class platform-catalog `Skill` / `SkillDocument` / `SkillKnowledgeCard` rows for ADR-079 professional Skills; Skill ownership/visibility is not derived from tenant workspace, and `Skill.category` currently stores the admin-selected Skill group key. **ADR-147 Slice S5a (landed locally, not deployed; initial parent audit rejected then repaired)** keeps `Assistant.roleId` plus `AssistantRoleSkill` as the only effective runtime/prompt/knowledge/invalidation Skill authority, with admin Role CRUD/full Skill-link replacement over `assistant_roles` / `assistant_role_skills`. User setup/recreate/Settings remain Role-first. Active direct Skill-selection APIs/contracts/MCP/plan Skill-count limits are removed; unused physical assignment rows remain only until S5b Release C and are never read or written by S5a code (Assistant FK cascade covers delete). Persisted unowned plan billing/limits JSON is preserved on ordinary Admin Plan updates without Admin/Public exposure.
