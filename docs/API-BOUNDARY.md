...
- turn-status route: `GET /api/v1/assistant/chat/web/turns/:clientTurnId`
  returns the durable logical-turn state (`unknown`, `accepted`, `running`,
  `completed`, `failed`, `interrupted`) plus committed user/assistant payloads
  where available. `unknown` means server acceptance is still unconfirmed and
  must never trigger same-payload redispatch. `accepted` or `running` means the
  client reattaches the exact `clientTurnId`. `completed` hydrates the durable
  turn under the same id. `failed` or `interrupted` is terminal and immutable
  for that id, so explicit Retry must mint a fresh `clientTurnId` (and fresh
  staged attachment identities when attachments are involved). Retry/Cancel
  actions bind only to the exact current pending user message id, never to
  historical failed rows. A restored pending slot may already be backed by a
  canonical server user row; client actions must preserve that failed row
  locally. If that restored canonical row had attachments after F5, Retry must
  not silently fall back to text-only without browser `File` objects.
...
- ordinary web USER_TURN recognition for live-present and wake exclusion is
  nullable-safe: `surfaceClient = null` counts as ordinary, and only the exact
  `surfaceClient = "async_continuation"` sentinel is excluded.
- web chat list/bootstrap rows expose compact `activeTurn` state plus optional
  `activeMediaJobs`, and `GET /api/v1/assistant/chats/web/:chatId/messages`
  returns committed history plus full `activeTurn` plus optional
  `activeMediaJobs`. While `activeTurn.status` is `accepted` or `running`, that
  attempt remains the authoritative continuity identity even if same-id history
  already contains an early assistant row; the history row may hydrate durable
  fields or attachments but must not clear busy state, unlock the composer, or
  demote the active send to terminal client truth.
...
- `apps/api` owns canonical message persistence, replay semantics, quota/media bookkeeping, and user-facing response shaping. ADR-161 text turns pass only validated `TextGenerationUsageAccountingV2` envelopes into Credits and ledger accounting; missing, unknown, or malformed text accounting fails closed and is not estimated or converted from a legacy carrier. Non-text usage contracts remain capability-specific. ADR-099 Session C keeps the immutable ledger write additive on high-confidence persisted paths: after the assistant reply is persisted, API appends `model_cost_ledger_events` rows for ordinary chat main replies and router/classifier entries, while successful background-task evaluator runs append one `background_task` row from their persisted usage snapshot. Subscription-period monthly media quota snapshots and mutations remain API-owned state backed by `workspace_media_monthly_quota_counters`.
...
- **ADR-088 Slice 1 + Slice 2 are now implemented.** The unified notification platform admin API is live at `/api/v1/admin/notifications/`:
  ...
  - **ADR-168/169 (assistant-sent email, separate from this notification pipeline):** the workspace connects its own mailbox (Mail.ru or Yandex) over OAuth under `/api/v1/assistant/integrations/email-mailbox` (`GET` read, `POST /connect` returns the provider authorization URL, `DELETE` disconnect); the provider redirect target `/api/v1/public/integrations/email-mailbox/callback` carries no Clerk session and is guarded solely by a single-use, expiring `state`. The workspace is always resolved server-side from the bearer identity and never accepted from the request. Runtime sends through `POST /api/v1/internal/runtime/email/send` (`assertPersaiInternalApiAuthorized`), which returns `sent` / `skipped:mailbox_not_connected|mailbox_token_invalid|provider_daily_limit_reached` / `failed` and performs **no** SMTP call when the workspace has no connected mailbox. Sending is now the customer's own mailbox over SMTP XOAUTH2 (`MailboxSmtpSendClientService`), not Postmark — `PostmarkEmailSendClientService` keeps its ADR-088 platform-notification role only. ADR-169 S5 deleted the ADR-168 `/api/v1/assistant/integrations/email-sender` routes (Postmark Sender Signatures verification) entirely; notification-platform request shape, policies, quiet hours, and dedupe remain unchanged.
...
- `POST /api/v1/internal/runtime/cross-session/resolve-carry-over-snapshot` — ADR-161 atomic first-writer-wins resolve for one Assistant-chat `cross_session_carry_over` snapshot. Accepts internal-token-authenticated `assistantChatId` plus a bounded string (empty is a valid frozen result) and returns the already-stored or newly-initialized exact bytes. Runtime never injects an unpersisted proposal into a provider request. The superseded `mark-carry-over-fired` cooldown endpoint is removed.
