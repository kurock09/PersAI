...
- `ADR-072` `T15-4` starts `runtime.workerTools` on the native runtime bundle as the shared worker baseline for current worker-class tools (`browser`, `image_generate`, `tts`, hidden internal `cron`). The block captures shared worker metadata such as family, outcome kind, timeout budget, confirmation policy, provider-routing support, and failure behavior so later browser/media/scheduler slices reuse one native worker layer instead of inventing tool-specific async semantics.
