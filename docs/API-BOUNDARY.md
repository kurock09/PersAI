...
- hard-stop route: `POST /api/v1/assistant/chat/web/stop` (body: `{ "clientTurnId": string }`). **Current:** response `204` always; in-memory registry on the stream-owning API pod only. **ADR-149 target:** durable cross-replica dispatch with honest responses (`200` `stopped` / `200` `already_done` / `404` `turn_not_found`); delete in-memory-only registry.
...
- the hard-stop route is idempotent. **Current:** returns `204` whether or not a matching in-flight turn exists; client treats it as fire-and-forget (miss cases are silent). **ADR-149:** explicit outcomes; terminal attempt `errorCode: "user_stopped"` on successful Stop; next-turn hydration includes explicit user-stop fact.
- **ADR-149 turn budgets (target):** web stream wall-clock (`PERSAI_RUNTIME_TURN_WALL_CLOCK_MS`) and progress-only idle stall (`PERSAI_RUNTIME_TURN_IDLE_STALL_MS`) replace `max(workerTimeouts)+15s` as the API stream ceiling. `video_generate` worker timeout applies only to the video job. Cadence `slow_avg` / `silent` remain disabled — do not re-enable in this program.
