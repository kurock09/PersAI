...
- `ADR-072` `T15-4/T15-5` now carry `runtime.workerTools` on the native runtime bundle as the shared worker baseline for current worker-class tools (`browser`, `image_generate`, `image_edit`, `tts`, `scheduled_action`, hidden internal `cron`). The block captures shared worker metadata such as family, outcome kind, timeout budget, confirmation policy, provider-routing support, and failure behavior so later browser/media/scheduler slices reuse one native worker layer instead of inventing tool-specific async semantics; `scheduled_action` is now the canonical product-facing contract, the native runtime executes its `create` / `list` / `pause` / `resume` / `cancel` surface through PersAI internal task registry/control endpoints rather than exposing raw scheduler triggers, and `apps/api` now persists native scheduled-action schedule/payload/claim state directly on PersAI task rows so hidden due-time pickup runs on a PersAI-owned scheduler loop with a global scheduler epoch that resets stale claims across rollout/startup instead of creating legacy cron jobs for new reminders. Native `image_edit` now reuses that same worker/media baseline with one current-turn reference image plus persisted artifact delivery, and hidden internal `cron` remains a separate internal-only compatibility seam.
...
- request body:
  ...
  - `keys.tool_image_generate` (optional write-only, shared by `image_generate` and `image_edit`)
  - `keys.tool_browser` (optional write-only)
  - `keys.tool_tts_elevenlabs` (optional write-only)
  - `keys.tool_tts_yandex` (optional write-only)
  - `keys.tool_tts_openai` (optional write-only)
  - `keys.tool_memory_search` (optional write-only)
  - `providers.tool_web_search`, `providers.tool_browser`, and `ttsPrimaryProviderId` when the active control-plane surface supports provider selection
- behavior:
  ...
  - tool credential refs are emitted into the materialized native runtime bundle during the next materialization/apply, with `tool_image_generate` reused for both `image_generate` and `image_edit`
