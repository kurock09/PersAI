...
- `ADR-072` `T15-4/T15-5` now carry `runtime.workerTools` on the native runtime bundle as the shared worker baseline for current worker-class tools (`browser`, `image_generate`, `tts`, `reminder_task`, hidden internal `cron`). The block captures shared worker metadata such as family, outcome kind, timeout budget, confirmation policy, provider-routing support, and failure behavior so later browser/media/scheduler slices reuse one native worker layer instead of inventing tool-specific async semantics; `reminder_task` now compiles as the `scheduled_action` family on that shared contract and the native runtime now executes its product-facing `create` / `list` / `pause` / `resume` / `cancel` surface through PersAI internal task registry/control endpoints rather than exposing raw scheduler triggers, while hidden internal `cron` remains the separate internal scheduler seam until later T15-5 follow-through replaces the legacy due-time pickup path.
- The native reminder executor now uses existing PersAI internal reminder boundaries instead of a provider-facing tool backend: `GET /api/v1/internal/runtime/tasks/items` resolves current task state, `POST /api/v1/internal/runtime/tasks/control` handles reminder mutations, and reminder create accepts clean native `conversationContext` for Telegram target routing while retaining `contextSessionKey` only as backend fallback compatibility.
- `POST /api/v1/internal/cron-fire` now acts only as the temporary legacy due-time trigger adapter. Replay safety and task-registry sync still live there for the callback path, but reminder summary cleanup, quota/render fallback, Telegram-vs-web routing, and web fallback delivery now flow through one shared PersAI-owned reminder delivery service so later native scheduler pickup can reuse the same delivery core unchanged.
- `ADR-072` `T15-4` now also gives `browser` its own typed native bundle surface: `runtime.browser` fixes the canonical tool code, credential tool code, supported provider ids, action list, and confirmation-required action set, while `tool_browser` on the PersAI admin Tool Credentials surface now materializes into `toolCredentialRefs.browser` for the hidden `Browserless` backend seam.
- `ADR-072` `T15-4` now also uses that native browser contract on the live request path: `apps/runtime` projects `browser` only when worker policy + Browserless credential + supported provider all agree, and `apps/provider-gateway` owns `POST /api/v1/providers/browser-action` for the bounded `snapshot | act` browser contract without exposing Browserless itself as the model-facing tool surface.
- Browser enable/disable and daily browser quotas still come from the existing plan-managed tool activation surface (`effectiveActivation` / `dailyCallLimit` compiled into native `toolPolicies`); `T15-4` does not introduce a separate browser-only plan/quota API.
...
- `ADR-072` `T15-3b` restores `web_search` and `web_fetch` as separate plan-tool executors instead of routing them through `knowledge_*`. Native `web_fetch` is landed on the Firecrawl seam, and native `web_search` now honors the full current control-plane provider set (`Tavily`, `Brave`, `Perplexity`, `Google (Gemini)`) on the shared native `query` / `count` contract with runtime-owned quota enforcement, internal secret resolution, and normalized untrusted-result metadata. Unknown provider ids still stay dark, and bounded live validation remains the next honest follow-through.
...
- DM access model:
  ...
  - connect creates a pending 6-digit owner-claim code
