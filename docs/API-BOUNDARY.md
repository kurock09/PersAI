...
- **ADR-088 Slice 1 + Slice 2 are now implemented.** The unified notification platform admin API is live at `/api/v1/admin/notifications/`:
  ...
  - **ADR-168 (assistant-sent email, separate from this notification pipeline):** workspace sender identity lives under `/api/v1/assistant/integrations/email-sender` (`GET` read + bounded Postmark re-check, `POST` request/replace address, `POST /resend`, `DELETE` remove); the workspace is always resolved server-side from the bearer identity and never accepted from the request. Runtime sends through `POST /api/v1/internal/runtime/email/send` (`assertPersaiInternalApiAuthorized`), which returns `sent` / `skipped:sender_email_not_verified` / `failed` and performs **no** Postmark call when the workspace has no verified sender. Both this path and `EmailChannelAdapter` now share one Postmark send transport (`PostmarkEmailSendClientService`); notification-platform request shape, policies, quiet hours, and dedupe are unchanged.
