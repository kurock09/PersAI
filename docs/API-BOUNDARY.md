...
- `ADR-072` `T15-4/T15-5` now carry `runtime.workerTools` on the native runtime bundle as the shared worker baseline for current worker-class tools (`browser`, `image_generate`, `tts`, `scheduled_action`, hidden internal `cron`). The block captures shared worker metadata such as family, outcome kind, timeout budget, confirmation policy, provider-routing support, and failure behavior so later browser/media/scheduler slices reuse one native worker layer instead of inventing tool-specific async semantics; `scheduled_action` is now the canonical product-facing contract, the native runtime executes its `create` / `list` / `pause` / `resume` / `cancel` surface through PersAI internal task registry/control endpoints rather than exposing raw scheduler triggers, and `apps/api` now persists native scheduled-action schedule/payload/claim state directly on PersAI task rows so hidden due-time pickup runs on a PersAI-owned scheduler loop with a global scheduler epoch that resets stale claims across rollout/startup instead of creating legacy cron jobs for new reminders. Hidden internal `cron` remains a separate internal-only compatibility seam.
