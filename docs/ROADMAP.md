...
6. Step 20 sandbox and attach-by-ref follow-through (`ready for live test`; local repo truth now has canonical `AssistantFile`, durable multi-pod workspace coordination, clean internal `files` execution including unified `files.send`, an atomic `files.write_and_send` happy path for create-and-deliver requests, and operator truth aligned with that model, while the final live surface proof is still pending)
