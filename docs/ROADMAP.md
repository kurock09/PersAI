...
3. memory, knowledge, cache, and model-routing cost architecture (`Slice A` and `Slice B` completed on the active path; `Slice C` is the next economics step)
...
2. Continue the memory/knowledge/cache/model-routing economics wave after Slice B completion by landing Slice C knowledge correction plus retrieval-model follow-through
3. Keep that economics wave honest about the current baseline (`pattern_only` retrieval and heuristic rerank are still current truth; provider-native cached input is now first-class on the active path, but retrieval-specialized follow-through is still not)
