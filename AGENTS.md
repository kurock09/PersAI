...
7. `docs/TEST-PLAN.md`
8. relevant `docs/ADR/*` for the task at hand. Migration history: `docs/ADR/072-persai-native-multichannel-runtime-replacement.md`. **Closed program archive** (do not treat as active backlog): `docs/ADR/078-consolidated-follow-through-program.md` through `docs/ADR/115-inbound-safety-program-contour-heuristics-and-async-moderation.md`. **Closure-mode ADR** (Slices landed, golden invariant locked, `cache-prefix rollout SHA` pending — do not reopen for new scope; only `cache-prefix rollout SHA` resolution remains): `docs/ADR/117-tool-instruction-source-of-truth-and-native-tool-runtime-selection-guide.md`. **Open program ADR:** `docs/ADR/118-skill-scenarios-and-model-owned-activation.md` (Skill scenarios as first-class admin-authored workflows + model-owned Skill activation via single `skill` tool; replaces hidden classifier + cadence + lexical-gate; execute its slices in order, Slices 1+2 must land in the same deploy; Slice 7 contributes one additive rule line to the canonical `tools` selection guide guarded by the ADR-117 golden test). Apart from ADR-118 (and the residual ADR-117 cache-prefix rollout) there is **no open orchestration program ADR** unless the user starts a new one.
