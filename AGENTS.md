...
Additional active parent-orchestrated program: `docs/ADR/161-provider-native-prompt-cache-discipline-and-usage-accounting.md` (Release A accepted at `03bacd5d`; final canonical accounting cutover deployed from `ce5d7f06`; 2026-07-21 cache-prefix repair deployed from `bfd800c5`; OpenAI assistant replay protocol hotfix implemented locally, hotfix deployment/live acceptance pending). The founder rejected B1/B2 because there are no commercial users: provider-gateway, runtime, and API converge directly on validated canonical v2 accounting, and temporary rollout flags, capability markers, Helm floors, probes, v1/missing-discriminator reads, and old text-accounting carriers are deleted. A bounded rolling overlap is accepted, but steady state has no v1 accounting compatibility path. The 2026-07-21 repair freezes one durable `cross_session_carry_over` snapshot per chat (including empty) and keeps plan-selected catalog tools byte-stable after `describe`; full contracts appear only in immutable tool results and a turn-local loaded set authorizes later calls. OpenAI assistant pre-tool text uses the official `EasyInputMessage` string shape, never assistant `input_text`. The original cache/prompt, long-loop savings, and provider-matrix closure requirements remain unchanged.
