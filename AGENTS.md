...
Additional active orchestration program: `docs/ADR/148-sandbox-session-warmth-and-fail-closed-cleanup.md` (opened 2026-07-14 — restore warm session-pod reuse after ADR-146 S3 over-retired every bound pod; control-plane fail-closed process cleanup; session-root pip/npm persistence; dependency-tree quota contour; deploy + live acceptance pending. ADR-146 remains closed; do not reopen it).
