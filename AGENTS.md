...
Additional active orchestration program: `docs/ADR/151-reusable-scripts-core-and-mcp-authoring.md` (**Accepted / Open; Domain + Admin API implemented and independently audited locally** on baseline `36947f75`; isolated PostgreSQL 16 full 189-migration proof and zero target-schema drift. **Scenario + Runtime block implemented and independently audited CLEAN locally** in a later session on the `adr-151-reusable-scripts` branch starting from checkpoint `cbeaccb4`: Scenario-step `scriptRef` materialization pin, internal runtime-facing `ScriptVersion` artifact read boundary with live re-authorization, provider-facing `script` tool dynamically projected only for an active step's materialized `scriptRef` and re-resolved before dispatch, Ajv 8.18 Draft-2020-12 input/output validation, server-derived `scriptInvocationKey`, atomic `SandboxJob` admission by `(assistantId, scriptInvocationKey)` with P2002 winner/loser replay and `idempotency_conflict`, and execution over the exact existing warm session sandbox with transient `/tmp`-only staging; the 2026-07-17 parent/independent-audit repair pass then added fail-closed materialization, required-nullable runtime `scriptRef`, sandbox admission/execution reauthorization and canonical hash/schema enforcement, proven transient cleanup with retirement, bounded per-invocation framing, safe Script cwd, mapping/schema hygiene, exact `LimitedCollector` retention, and real package-gate coverage. The final allowed-model re-audit returned CLEAN; only deployed Kubernetes, true concurrent PostgreSQL admission, and model-driven warm-session probes remain live-only. **Admin UI + MCP authoring block implemented and independently audited CLEAN locally 2026-07-17**: Admin Scripts page patterned after Admin Roles (localized metadata CRUD, draft/published/archived lifecycle, immutable published-version history, draft code/manifest/schema authoring with save/validate/publish, typed conflict/error states, Skill-bindings ordered full-replace via the existing `GET`/`PUT .../skills/{skillId}/scripts` routes — no invented runtime-smoke route), a Scripts entry in the Admin navigation, nine thin typed MCP tools (`script_list`/`script_get`/`script_upsert`/`script_version_upsert`/`script_version_validate`/`script_publish`/`script_archive`/`skill_scripts_list`/`skill_scripts_replace`) resolving immutable `scriptKey` exactly like existing `role_*` tools, and an optional `scriptRef` field on the existing `skill_scenario_upsert` step schema. Parent/audit repairs enforce exact validation/trim parity, PATCH-before-validate/publish, stale-response ownership, guarded version loading, and full binding-control locking; 18 web tests and 39 full Admin MCP tests (22 Script-focused) are green.). It establishes platform-global reusable Script + immutable published ScriptVersion, ordered full-replace Skill↔Script links, bounded `ScenarioStep.scriptRef` input mapping, and synchronous active-step-only `script.execute` over the exact existing warm session sandbox path. ADR-147 Role-only effective Skills remains unchanged. No ScriptRun, Tool SDK, browser executor, async/jobRef/wait/notify, or managed secrets: ADR-152/153 own those. Invocation truth must be the existing `SandboxJob` with exact nullable `scriptVersionId`, `scriptSkillId`, `scriptContentHash`, `scriptInvocationKey`, validated policy snapshot, and nullable unique `(assistantId, scriptInvocationKey)` idempotent admission. No Script-specific pod, NetworkPolicy, image, package restriction, or storage layer. Remaining work is the final repository gate and deployment/live acceptance. Parent orchestrator/auditor delegates bounded slices; deployment/live acceptance only at final gate.
...
re-audit returned CLEAN. Deployment and live acceptance remain pending.
