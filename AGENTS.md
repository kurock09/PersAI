...
Additional active orchestration program: `docs/ADR/147-assistant-roles-and-effective-skills.md` (in progress 2026-07-14 — S0 accepted; S1 schema/expand, S2 role-only API/runtime/prompt, S3 user Role UX, S4 Admin Role constructor/MCP, and S5a Release-B contract cutover implemented and parent-audited CLEAN; S5a required two rejected audits before final fail-closed gate hardening; S5b physical contract/drop implemented locally against `d8195d1d`, two focused-evidence audits rejected then repaired, final re-audit CLEAN, isolated 189-migration/semantic fixture proof PASS, complete local repository gate PASS, Release C not deployed — idempotent plan JSON Skill-limit cleanup then drop of `assistant_skill_assignments` / `AssistantSkillAssignmentStatus`. Every Assistant, including each Assistant in a B2B multi-assistant account, has one required Role; effective Skills come only from that Role while existing Skill/Scenario/Knowledge/runtime mechanics remain intact. Seven bounded slices S0-S6; safe expand -> role-only -> contract/drop rollout; parent orchestrator/auditor with one sequential implementation slice at a time. Cursor Grok 4.5 is the default subagent; GPT-5.4 is used only when stronger reasoning is necessary or Grok fails. Release B `a225143e` is deployed through healthy bot pin `4c28dd52`; CI, migration, exact-revision inventory, and operator-accessible Role/runtime/MCP acceptance passed. Authenticated B2B acceptance proved three independently addressed Assistants and live `Маркетолог · Instagram-карусель` header indication; repair commit `d8195d1d` is present. Commit/one final push precede migration-gated Release C deploy. Existing Cursor chats retain their attached old MCP schema; reload the server then open a new Agent chat. User/Admin UI must be fully RU/EN, concise, premium, and system-consistent; Admin Roles follows Admin Skills).
