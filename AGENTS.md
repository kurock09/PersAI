...
Additional active orchestration program: `docs/ADR/151-reusable-scripts-core-and-mcp-authoring.md` (**Accepted / Open; Domain + Admin API implemented and independently audited locally** on baseline `36947f75`; isolated PostgreSQL 16 full 189-migration proof and zero target-schema drift). It establishes platform-global reusable Script + immutable published ScriptVersion, ordered full-replace Skill↔Script links, bounded `ScenarioStep.scriptRef` input mapping, and synchronous active-step-only `script.execute` over the exact existing warm session sandbox path. ADR-147 Role-only effective Skills remains unchanged. No ScriptRun, Tool SDK, browser executor, async/jobRef/wait/notify, or managed secrets: ADR-152/153 own those. Invocation truth must be the existing `SandboxJob` with exact nullable `scriptVersionId`, `scriptInvocationKey`, validated policy snapshot, and nullable unique `(assistantId, scriptInvocationKey)` idempotent admission. No Script-specific pod, NetworkPolicy, image, package restriction, or storage layer. Remaining implementation blocks are scenario+runtime, Admin+MCP, and final audits/gates; runtime/security still requires independent allowed-model audit. Parent orchestrator/auditor delegates bounded slices; deployment/live acceptance only at final gate.
