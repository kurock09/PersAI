...
Additional active parent-orchestrated program: `docs/ADR/161-provider-native-prompt-cache-discipline-and-usage-accounting.md` (Release A accepted at `03bacd5d`; canonical accounting cutover deployed from `ce5d7f06`; cache-prefix repair deployed from `bfd800c5`; OpenAI assistant replay protocol repair deployed from `5011e905`; shared append-only tool-history repair and D2a DeepSeek durable append-trace implemented locally, deployment/live acceptance pending). The founder rejected B1/B2 because there are no commercial users: provider-gateway, runtime, and API converge directly on validated canonical v2 accounting, and temporary rollout flags, capability markers, Helm floors, probes, v1/missing-discriminator reads, and old text-accounting carriers are deleted. The cache repairs freeze one durable `cross_session_carry_over` snapshot per chat, keep plan-selected catalog tools byte-stable after `describe`, preserve base messages byte-for-byte through tool loops, and project every assistant tool turn retained by canonical history into one deterministic compact cross-turn replay. Canonical exchanges remain full; compact replay participates in the ordinary hydration/durable-compaction budget. OpenAI assistant pre-tool text uses the official `EasyInputMessage` string shape, never assistant `input_text`. DeepSeek ordinary/deep chat uses a server-only append-only epoch with pre-dispatch budget finalization; OpenAI/Anthropic keep the generic sealed-spine protocol. The original cache/prompt, long-loop savings, and provider-matrix closure requirements remain unchanged.
