...
Additional active parent-orchestrated program: `docs/ADR/161-provider-native-prompt-cache-discipline-and-usage-accounting.md` (Release A deployed and founder-accepted at `03bacd5d`; final canonical maintenance cutover implemented locally, deployment/live acceptance pending). The founder rejected B1/B2 because there are no commercial users: provider-gateway, runtime, and API converge directly on validated canonical v2 accounting, and temporary rollout flags, capability markers, Helm floors, probes, v1/missing-discriminator reads, and old text-accounting carriers are deleted. A bounded rolling overlap is accepted, but steady state has no v1 accounting compatibility path. The original cache/prompt, long-loop savings, and provider-matrix closure requirements remain unchanged. Do not push or deploy unless explicitly asked.
