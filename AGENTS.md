...
Additional active repair program: `docs/ADR/166-unified-live-presentation-state-machine.md` (local CLEAN 2026-07-27 on baseline `1f2512a8`; founder-authorized commit + push in this entry; deploy/live acceptance pending). One live USER_TURN owns a media series; same-id history cannot demote the live overlay; open-turn media is explicit-claim-gated; catch-up becomes visible only after runtime acceptance + publish bind; terminal full snapshots clear Working; Stop cannot requeue delivered inline media; compact live rail replaces the fixed `8.75rem` blank reserve, while a per-message progressive height high-water prevents long-loop shrink/bounce without up-front empty space. Three-image order/out-of-order, Working `3→2→1→0`, F5/soft-detach, busy/retry, failure, terminal discovery, mobile height-stability, and cross-thread history/restore races are covered. Independent re-audits and full CI-like local gates are CLEAN. Parent orchestrates/audits/commits; because the repair crosses active ADR-162/165, implementation subagents use **only** `cursor-grok-4.5-high-fast`.
