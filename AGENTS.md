...
Additional active parent-orchestrated program: `docs/ADR/169-mailbox-connected-assistant-email.md` (opened 2026-08-01 on baseline `9779dae6`; documentation only at opening). Assistant-sent customer mail leaves through the customer's **own** mailbox over OAuth + SMTP XOAUTH2 (Mail.ru `userinfo`/`mail.imap` and Yandex `mail:smtp` in v1; Google deferred to Gmail API `gmail.send` because SMTP needs the restricted `https://mail.google.com/` scope). This supersedes only ADR-168's sender-verification layer — Postmark Sender Signatures, the Account token, and address-confirmation polling are deleted, while the `email_send` tool contract, plan limits, per-turn cap, skip+guidance, audit, internal endpoint, and Integrations card survive. Postmark keeps its ADR-088 platform-notification role and is never used for customer mail. Tokens are stored via `PlatformRuntimeProviderSecretStoreService` under `mailbox_oauth:${workspaceId}`, **not** the per-assistant `persai.secretRefs.v1` envelope. The connect-initiate route must be in `CLERK_AUTHENTICATED_ROUTES`; the provider redirect target must not be, and guards itself with a single-use `state`. Parent orchestrates/audits/commits; implementation subagents use `claude-sonnet-5-thinking-high`. One push at the end after audits and the full gate.
