...
Additional active orchestration program: `docs/ADR/147-assistant-roles-and-effective-skills.md` (in progress 2026-07-14 — S0 accepted; S1 schema/expand, S2 role-only API/runtime/prompt, S3 user Role UX, S4 Admin Role constructor/MCP, S5a Release-B contract cutover, and S5b physical contract/drop implemented, parent-audited CLEAN, and deployed. S5b release `a11c8b6b` passed isolated 189-migration proof, complete local/GitHub gates, migration-approved bot pin `05ccaed4`, Argo/exact-image/health/live DB acceptance, and refreshed user Settings Role visual. Every Assistant, including each Assistant in a B2B multi-assistant account, has one required Role; effective Skills come only from that Role while existing Skill/Scenario/Knowledge/runtime mechanics remain intact. Seven bounded slices S0-S6; safe expand -> role-only -> contract/drop rollout; parent orchestrator/auditor with one sequential implementation slice at a time. Cursor Grok 4.5 is the default subagent; GPT-5.4 is used only when stronger reasoning is necessary or Grok fails. Release B `a225143e` is deployed through healthy bot pin `4c28dd52`; authenticated B2B acceptance proved three independently addressed Assistants and live `Маркетолог · Instagram-карусель` header indication. S6 remains open only for one authenticated Admin preview visual and a new-Agent-chat MCP catalog check: the current ordinary B2B Clerk session has no Admin entry and redirects `/admin/roles` to `/app`; existing Cursor chats retain their attached old MCP schema, so reload the server then open a new Agent chat. User/Admin UI must be fully RU/EN, concise, premium, and system-consistent; Admin Roles follows Admin Skills).
