...
Additional active parent-orchestrated program: `docs/ADR/161-provider-native-prompt-cache-discipline-and-usage-accounting.md` (founder-prioritized docs-only opening 2026-07-20; long-loop refinement independently audited CLEAN with zero P0/P1/P2; implementation authorized). The compiled stable system prefix is verified stable; for ordinary/deep chat, the early volatile developer placement defect is DeepSeek-only. Scope is a clean provider-native cutover for DeepSeek stable-history placement, an immutable compact exchange spine with newest-three full suffix overlays, Anthropic stable + latest-sealed breakpoints, measured OpenAI routing-key semantics plus model-declared pre-5.6 policy and validated GPT-5.6+ explicit stable/spine boundary blocks, canonical non-overlapping `total/uncached/write/read` cost and Credits accounting, exact tool projection-family identity, actual-vs-no-cache provider-input cost, and content-safe observability. ADR-161 narrowly supersedes ADR-156 in-turn aging: compact spine at first insertion + newest-three full overlays, with no later in-turn masking; cross-turn windows and compact/full definitions stay unchanged. Closure requires a sequential 50-iteration benchmark per active provider/model-family policy and one authenticated 40–50-tool PersAI turn proving growing cache reads and strictly positive net provider-cost savings; hit percentage alone fails. No unrelated prompt-semantic change, speculative commit attribution, universal 90% gate, feature flag, permanent legacy serializer, or permanent dual-read alias. Parent orchestrates/audits/commits; complex implementation uses `gpt-5.6-terra-medium`, simple implementation uses `gpt-5.4-medium`; Sol and Opus subagents are forbidden. Do not proliferate artificial slices. Cross-service usage v2 uses a bounded consumer-first versioned rollout and deletes v1 only after old producers drain. Push exactly once after implementation, audits, and full gates; push triggers deploy.
