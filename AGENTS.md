...
Additional active orchestration program: `docs/ADR/147-assistant-roles-and-effective-skills.md` (in progress 2026-07-14 — S0 accepted; S1 schema/expand and S2 role-only API/runtime/prompt cutover implemented locally and parent-audited CLEAN; S3 next. Every Assistant, including each Assistant in a B2B multi-assistant account, has one required Role; effective Skills come only from that Role while existing Skill/Scenario/Knowledge/runtime mechanics remain intact. Seven bounded slices S0-S6; safe expand -> role-only -> contract/drop rollout; parent orchestrator/auditor with one sequential implementation slice at a time using the founder-approved GPT-5.4 / Cursor Grok 4.5 / Composer pool selected by slice risk. S1-S5 remain local; complete ADR-146-grade repository gates precede one final push, then parent-supervised deploy and B2C/B2B/Admin/MCP live acceptance. User/Admin UI must be fully RU/EN, concise, premium, and system-consistent; Admin Roles follows Admin Skills).
